...
To summarize, we cannot reliably detect named entities by looking them up in a gazetteer,
and it is also hard to develop rules that will correctly recognize
ambiguous named entities on the basis of their context of occurrence.
Although lookup may contribute to a solution, most
...
graphical interface ``nltk.app.chunkparser()``.
...
In Section sec-applications-of-tagging_ we saw how we could interrogate
...
sentence from the Wall Street Journal tagged files.
    ...
    >>> cp = nltk.RegexpParser(grammar)
    >>> chunkscore = nltk.chunk.ChunkScore()
    >>> for filename in nltk.corpus.treebank_chunk.files()[:5]:
    ...     for chunk_struct in nltk.corpus.treebank_chunk.chunked_sents(filename):
...
#. |easy| **Chunking Demonstration:**
   Run the chunking demonstration: ``nltk.app.chunkparser()``

#. |easy| **Chunk Grammar Development:**
   Try developing a series of chunking rules using the graphical
   interface accessible via ``nltk.app.chunkparser()``
...
#. |hard|
   ...
   ``for sent in nltk.corpus.treebank_chunk.chunked_sents(filename)``.  These are flat trees,
