...
clearly a more difficult task. Unlike Table db-locations_, ex-ie4_
contains no regular structure for associating organization names with
location names. As we will see in Chapter chap-semantics_, one
approach to this kind of problem involves building a very general
representation of meaning. However, for the time
being we will adopt a different strategy, where we decide in
...
As we pointed out in section sec-ch07-introduction_, Information
Extraction is usually broken down into at least two major steps:
`Named Entity Recognition`:dt: and `Relation Extraction`:dt:. Named
Entities (|NE|\ s) are noun phrases that denote
