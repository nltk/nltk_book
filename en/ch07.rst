...
====================================
8. Context Free Grammars and Parsing
====================================
...
.. TODO: include overview of cfg.Grammar and cfg.Production in 8.4
.. TODO: put dependency grammar material in later chapter?
...
popular`:lx: as a means of identifying which of several possible
...
Earlier chapters focused on words: how to identify them, how to
analyze their morphology, and how to assign them to classes via
...
linguistic regularities that cannot be described simply in terms of
n-grams.
...
Listing indent-tree_ prints a tree object using whitespace formatting.
...
said to be :dt:`recursive` if a category occurring on the left hand
...
.. [#] It should be borne in mind that it is possible to create
   examples that involve 'non-standard' but interpretable
...
#. |soso| Pick some common verbs and complete the following tasks:
   ...
   a) Write a program to find those verbs in the Prepositional Phrase Attachment Corpus
      ...
      Section sec-syntactic-ambiguity_).
...
#. |hard| Write a function that takes a grammar (such as the one defined in
   Listing cfg_) and returns a random sentence generated by the grammar.
...
#. |soso| Compare the performance of the top-down, bottom-up, and left-corner
   ...
   parser takes on the same sentence (Section sec-timing_).  Write a function that runs all
...
projects that have achieved interesting and impressive results in
