...
Serial No. 700,000. In addition, there is a relation between the two
entities, namely the relation which is expressed as *gef\ |aumlaut|\
llt* in German and as *likes* in English. If you understand the
meaning of ex-sem1_, then you know that it is true in situation *s*. 
...
sensible conversations. Suppose you are having a chat session with a
person and a computer, but you are not told at the outset which is
which. If you cannot identify which of your partners is the computer
after chatting to each of them, then the computer has successfully
imitated a human. Then, according to Turing, we should be prepared to
say that the computer can think and be said to be intelligent. Now, if
you accept that a computer that passes the Turing test is intelligent,
you should also accept that the computer understands natural language.
...
Pursuing this approach further, consider the following task: we type in
a query in English, and the computer responds with an appropriate (and
if we are lucky, correct) answer. Building software that allows this
task to be carried out in general way is extremely hard, but solving
it for a particular case is close to trivial. We start off by assuming
that we have some relevant data in a structured form. To be concrete,
we use a database table whose first few rows are as follows:
...
.. table:: cities
    ...
    Table of cities, countries and populations

Next, we build a toy grammar to parse English queries.

.. pylisting:: sql-fcfg
   :caption: Toy grammar that outputs SQL
    
    >>> nltk.data.show_cfg('grammars/sql.fcfg')
    % start S
...
This allows us to parse a query into SQL.
    ...
    >>> cp = load_earley('grammars/sql.fcfg')
    >>> query = 'Which cities are in USA'
    >>> trees = cp.nbest_parse(query.split())
    >>> answer = trees[0].node['sem']
    >>> q = join(answer)
    >>> print q
    SELECT City FROM city_table WHERE Country="united_states"

Finally, we execute the query over the database.

    >>> from nltk.sem import chat80
    >>> rows = chat80.sql_query(path, q)
    >>> for r in rows: print "%s" % r,
    chicago detroit los_angeles new_york philadelphia
