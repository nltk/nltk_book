...
By placing an `existential quantifier`:dt :|exists|\ `x`:math: in
front of ex-predlog52_, we can `bind`:dt: these variables, as in
ex-predlog61_, which means ex-predlog62_ or, more idiomatically,
ex-predlog63_.
...
``astring`` by ``'8'``, even though there may in fact be no such
occurrences.
