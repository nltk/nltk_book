...
.. ex::
   .. _ex-dbq01:
   .. ex:: Which country is Caracas in?
   .. _ex-dbq02:
   .. ex:: Venezuela.

Building software that allows this
...
that we have data about cities and countries in a structured form. To be concrete,
we will use a database table whose first few rows are as follows:
...
writing queries in a database query language such as SQL. Thus, rather
than using the English query ex-dbq01_, we might have used ex-dbq1_ to
pull out the value ``'venezuela'``.

.. _ex-dbq1:
.. ex::
   SELECT Country FROM city_table WHERE City = 'caracas'

.. Note::
   If you want to find out more about SQL,
   `<http://www.w3schools.com/sql/>`_ is a convenient online reference.

But we can also translate English queries into SQL. 
