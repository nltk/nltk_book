...
In Section log-semantics-introduction_, we already touched on two
...
propositional logic are `propositional variables`:dt:, which we shall
...
(encoded via the functions ``AND()`` and ``IMPLIES()`` respectively):
    ...
    ...	        print "%s %s => %s" % (first, second, m.AND(first, second))
    True True => True
    True False => False
    False True => False
    False False => False
    >>> for first in [True, False]:
    ...     for second in [True, False]:
    ...	        print "%s %s => %s" % (first, second, m.IMPLIES(first, second))
    True True => True
    True False => False
    False True => True
    False False => True
...
truth values we assigned to propositional variables such as ``'P'``,
``'Q'`` and ``'R'``.
...
this case, it is `bound`:dt: by the indefinite `NP`:gc: `a
poodle`:lx:, and this is a different relationship than
...
logical rendering in effect says that *if* something is a poodle, then
...
In Section sect-fine-grained-selection-of-words, we pointed out that
mathematical set notation was a helpful method of specifying
properties `P`:math: of words that we
wanted to select from a document. We gave the example ex-set-comprehension-math2_, 
which we glossed as "the set of all `w`:math: such that `w`:math: is an
...
We have a special name for this: `lambda abstraction`:dt:. When one
initially encounters |lambda|-abstracts, it
can be hard to get an intuitive sense of their meaning. One English
gloss for ex-walk-chewgum12_ is "be an `x`:math: such that  `x`:math:
walks and `x`:math: chews gum". Within the tradition of formal
semantics of natural language, it has often been suggested that
|lambda|-abstracts are good representations for verb phrases (or
subjectless clauses),
particularly when these occur as arguments in their own right. This is
illustrated in 
ex-walk-chewgum2_.
...
.. ex::
   .. _ex-walk-chewgum21:
   .. ex:: Walking and chewing-gum is hard
...
addition, we make explicit the types of the expressions involved.
   ...
   1. If `P`:math: is a predicate of type e^n -> t, and |tau|\ 1,
      ... |tau|\ n are terms of type e, then P(|tau|\ 1,
      ... |tau|\ n) is of type t.
