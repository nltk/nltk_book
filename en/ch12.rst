...
particular approach to this problem, but at various points we shall
...
sentence, you might say that ex-sem1 means the same as ex-sem2_:
...
did't already understand English, we would be stuck! Nevertheless,
...
*s* with are two entities, Stefan and a specific camera, say Leica M3
...
relation. You also know that the subject of `gef\ |aumlaut|\ llt`:lx:
plays the role of the object that is liked, while the object plays the
role of the agent who does the liking.
...
the sense of being able to say that ex-sem1_ and ex-sem2 'have the
same meaning'. In the second case, rather than relating ex-sem1_ to
something made up of symbols, we related it to a situation in the
...
ex-sem1 could be combined to make up the meaning of the whole.
...
'understands' English, it just needs to behave as though it did. 
What is important here is not so much the specifics of Turing's
imitation game, but rather the proposal to judge 'understanding'
in terms of observable behaviour on the part of the computer.
...
understanding. We will look at one in particular: we type in
...
Building software that allows this task to be carried out in general
...
database table whose first few rows are as follows:
...
.. Note:: The data illustrated above is drawn from the Chat-80 system
   described by [Warren1982EEA]_, which will be further discussed
...
writing queries in a database query language such as SQL. For example,
executing the query ex-dbq1_ will pull out the value ``'greece'``:
...
.. Note::
     ...
     `<http://www.w3schools.com/sql/>`_ is a convenient online reference.
...
SQL. The grammar ``sql0.fcfg`` illustrates how we can assemble a
meaning representation for a sentence by in
...
chapter) to splice the values for the daughter constituents so as to
...
queries such as `What cities are located in China?`:lx:. The grammar
``sql0.fcfg``, together with the NLTK Earley parser, has been
instrumental in carrying out the translation. The grammar is a mixed
bag. On the one hand, it assigns some kind of meaning to the parts of
sentences. For example, `what cities`:lx: receives the meaning
...
sort. Nevertheless, the grammar is, to be blunt, a hack. First off, we
...
field names, in which case the grammar would crash and burn. Equally,
...
our English queries into an XML query language. 
...
.. note:: |TRY|
   ...
   ex-dbq21_ into ex-dbq22_, and check the return values given by
   executing the query. You will probably find it easiest to first
   extend the grammar to handle queries like `What cities have
   populations above 1,000,000`:lx:. After you have had a go at this,
   you can compare your solution to ``grammars/sql1.fcfg`` in the NLTK
   data distribution.
...
interpretation for `and`:lx: returns to an idea we mentioned earlier,
...
interpretation from `propositional logic`:dt:
