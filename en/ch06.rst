...
In the context of a longer program, we would also need a second ``print``
statement after the loop, to finish the line with a newline character.
A third solution is more compact again, but possibly less readable:
...
section of the Brown Corpus, and for each sentence, iterates over each word.
    ...
    >>> [word for word in nltk.corpus.brown.words(categories='news')
    ...     if len(word) == 17]
...
variable, but use it directly as an argument to the ``average()`` function.
    ...
    >>> average([len(word) for word in nltk.corpus.brown.words(categories='news')])
    4.40154543827
...
of language processing tasks (e.g. Section sec-dictionaries_).
...
.. pylisting:: print-freq
   ...
   def print_freq(tokens, num=50):
       ...
       cumulative = 0.0
       for rank, word in enumarate(fd.keys()[:num]):
           cumulative += fd[word] * 100.0 / fd.N()
           print "%3d %3.2d%% %s" % (rank+1, cumulative, word)

   >>> print_freq(nltk.corpus.brown.words(categories='news'), 20)
     1  05% the
     2  10% ,
     3  14% .
     4  17% of
...
.. pylisting:: modal-tabulate
   ...
   def tabulate(cfdist, words):
       print 'Genre ', ' '.join([('%16s' % w) for w in words])
       for genre in sorted(cfdist.conditions()):           # for each genre
           print '%-16s' % genre,                          # print row heading
   ...
   news                94     86     66     36     50    387
   religion            84     59     79     12     54     64
   hobbies            273     59    130     22     83    259
   government         115     37    152     13     99    237
   adventure           48    154      6     58     27     48
