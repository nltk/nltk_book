...
inclined to make heavy use of loop variables, and no use of list comprehensions.
In this section we'll see how avoiding loop variables can lead to more readable
...
However, a better solution uses two list comprehensions as shown below.
We sacrifice some efficiency by having two passes through the data,
but the result is more transparent.
...
second solution found all of the longest words.  In most cases we actually prefer
to get all solutions, though its easy to find them all and only report one back to the user.
In contrast, it is difficult to modify the first program to find all solutions.
...
through the data is very fast.  We also need to balance our concerns about
...
Our use of list comprehensions has helped us avoid loop variables.  However,
...
This code shows that when we write ``bar = foo``,
...
When we modify something inside ``foo``, we can see
...
Thus, the line ``bar = foo`` does not copy the contents of the
...
This behavior extends to other aspects of the Python language.  In
Section sec-functions_ we will see how it effects the way parameters
are passed into functions.  Here's an example of how it applies to
copying:
...
This last step modified the object references, but not the objects themselves.
The ``['Python']`` object wasn't changed, and is still referenced from two places in
...
Discuss key points of ``http://www.python.org/dev/peps/pep-0008/``
...
This is the parenthesized list of variables and constants following
...
We will address some closely related issues in our discussion
of variable scope later in this section.
...
better approach, known as "duck typing" in Section sec-object-oriented_.)
...
Sometimes list comprehensions get cumbersome, since they
can mention the same variable many times, e.g.:
``[word for word in sent if property(word)]``.
We can perform the same task more succinctly as follows:
...
The function ``is_lexical(word)`` returns ``True`` just in case
...
equivalent to ``[item for item in seq if f(item)]``.
...
The higher order functions like ``map`` and ``filter`` are certainly useful, but
in general it is better to stick to using list comprehensions since they are
often more readable.
...
scalability tricks (e.g. replace each word with an integer); Figure strings-to-ints_
...
representation.  We will use the same format as the Penn Treebank,
a combination of brackets and labels:
...
Here, the node value is a constituent type (e.g., `np`:gc: or `vp`:gc:),
and the children encode the hierarchical contents of the tree.
...
[concept of indexing; using storage to save time...]

Consider the problem of finding reduplicated words in a lexicon.
In particular, let's find words *w* such that *w+w* is also a word.
A brute-force solution is to iterate over every word, form its
reduplicated version, then try to find it by iterating over the
dictionary.  This approach would do `n`:math:\ `2`:superscript:
key comparisons, where `n`:math: is the number of lexical entries.
A much faster method is to convert the lexicon from its original
list structure into a Python dictionary:

    >>> from nltk import Index
    >>> from nltk.corpus import toolbox
    >>> def english_glosses(entry):
    ...     content = Index(entry)
    ...     return '; '.join(content['ge'])
    ...
    >>> lexgloss = dict((lexeme, english_glosses(entry))
    ...                 for lexeme, entry in toolbox.entries('rotokas.dic'))
    >>> for lex in sorted(lexgloss)[:10]:
    ...     if lex+lex in lexgloss:
    ...         print "%s (%s); %s (%s)" %\
    ...             (lex, lexgloss[lex], lex+lex, lexgloss[lex+lex])
    ...
    kae (blow); kaekae (tempt)
    kape (unable to meet); kapekape (embrace; grip with arms not meeting)
    kapi (beak; bill); kapikapi (bow of ship; peak of house)
    kapo (fasten.cover.strip); kapokapo (fasten.cover.strips)
    kare (return); karekare (return)
    kari (rip; tear); karikari (tear; shred)
    karu (open; unlock; untie; unhook); karukaru (open)
    kasi (burn); kasikasi (angry)
    kau (solid; hard); kaukau (sweet potato)
...
.. _sec-object-oriented:

..
   Efficiency topics, e.g.:
