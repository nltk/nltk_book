...
Finally, line sequence4_ discards the length
...
half; if its later we use the second half (this is sometimes called
`decrease-and-conquer`:dt: since we actually decreased the size of the problem
with each step).
...
the list, then look for adjacent identical items.
...
Building Deeply Nested Objects (notes)
--------------------------------------

We can use recursive functions to build deeply-nested objects.
For example, a `letter trie`:dt: is a method for indexing
a lexicon, one letter at a time.  For example, if ``trie``
held a letter trie, then ``trie['c']`` would be a smaller
...
We can sometimes speed up the execution of a program by building some auxiliary data structure
such as an index [MORE].  Here's an example of a simple text retreival system for the Movie Reviews
Corpus.  By indexing the document collection it can provide much faster lookup.
...
Any language models can now work with integers...  [further discussion and performance
comparison.]
