...
convenient ways to access these corpora, and has data packages containing corpora
and corpus samples (for a listing, see ``http://www.nltk.org/data``).
We will discuss some of these these in later chapters.
...
Tasks such as word sense disambiguation (Section sec-automatic-natural-language-processing_)
...
that loads the corpus and then scans through it for any occurrences of
the linguistic pattern you are interested in.
