...
.. TODO: dealing with binary files (e.g. MSWord)
.. TODO: dealing with PDF files (using pyPdf library)
...
The most important source of texts is undoubtedly the Web.  Its nice to have
existing text collections to explore, such as the corpora we saw in
Chapter chap-introduction_.  However, you probably have your own text sources
in mind.

The goal of this chapter is to answer the following questions:

#. how can we write programs to access text on the web, in order
   to get hold of an unlimited range of language material?
#. how can we split documents up into individual words and
   punctuation symbols, in order to do the same kinds of
   analysis we did with text corpora in Chapter chap-introduction_?
#. what new features of the Python programming language are
   needed to do this?

In order to address these questions, we will look at more
...
so much text on the web is in HTML format, we will see how to
dispense with markup.
...
The read() process will take a few seconds as it downloads a large book.

    >>> text = nltk.Text(nltk.wordpunct_tokenize(raw))
    >>> len(text)
    255809
    >>> text[1020:1060]
...
.. note:: It is possible to use the methods described above
...
For a more extended treatment of regular expressions, see app-regexps_.
