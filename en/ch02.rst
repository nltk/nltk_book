...
Perhaps the most popular electronic lexical resource is known as a `wordnet`:dt:, a sophisticated kind of
`semantic network`:dt:.  |NLTK| includes the English WordNet, and it is
...
Each synset is characterized by a set of synonymous words:
    ...
    >>> dish4_lemmas = wordnet.synset('dish.n.04').lemmas
    >>> [lemma.name for lemma in dish4_lemmas]
...
WordNet is an important resource for many areas of Natural Language Processing.
We will discuss it in more detail in Section wordnet_.
...
WordNet 3.0 data is distributed with NLTK, and includes 155,287 words
and 117,659 synsets.
