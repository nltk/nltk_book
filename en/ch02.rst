...
makes use of this simpler syntax:
    ...
    ...     num_vocab = len(set(w.lower() for w in gutenberg.words(filename)))
    ...     print num_chars/num_words, num_words/num_sents, num_words/num_vocab, filename
    ... 
...
An interesting property of this collection is its time dimension:
    ...
    >>> [filename[:4] for filename in inaugural.files()]
...
`Citizens`:lx:.  The result is shown in Figure fig-inaugural2_.

    >>> cfd = nltk.ConditionalFreqDist((target, file[:4])
    ...           for filename in inaugural.files()
    ...           for w in inaugural.words(filename)
...
.. table:: tab-corpus
   ...
   ``abspath(filename)``            the location of the given file on disk
...
contains the year as the first four characters.
    ...
    >>> cfd = nltk.ConditionalFreqDist((target, filename[:4])
    ...           for filename in inaugural.files()
    ...           for w in inaugural.words(filename)
    ...           for target in ['america', 'citizen']
    ...           if w.lower().startswith(target))
...
Summary of Conditional Frequency Distributions
----------------------------------------------
...
produced by the following code:
    ...
    >>> cfd = nltk.ConditionalFreqDist((filename, name[-1])
...
.. note::
   The above code sample has some important subtleties.  The second line uses
   the ``int()`` function to convert characters to integers, thus the
   character ``"1"`` for primary stress is converted to an integer ``1``.
   To convince yourself of the difference between these two representations of
   the number one, type the following at a Python prompt: ``"1" * 7``
   and ``1 * 7``.  Another subtlety of the above program is that our
...
Tasks such as word sense disambiguation (Section sec-automatic-natural-language-understanding_)
and semantic interpretation more generally (Chapter chap-semantics_) depend on lexical
resources containing rich semantic information.  WordNet is the most widely used
lexical semantic resource and is discussed in Section sec-wordnet_.
Other important resources are VerbNet and PropBank, both supported in NLTK. 
We will discuss these further in chapter [REF].
...
#. |soso| Write the slice expression to produce the last two
   words of ``text2``.
