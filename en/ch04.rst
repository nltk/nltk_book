...
analysis might show mistakes in the gold standard.
...
we train a unigram tagger, use it to tag a sentence, then evaluate:
    ...
    >>> unigram_tagger = nltk.UnigramTagger(brown_news)
...
testing on the remaining 10%:
    ...
    >>> size = int(len(brown_news) * 0.9)
    >>> brown_news_train, brown_news_test = brown_news[:size], brown_news[size:]
...
First we train it, then use it to tag untagged sentences:
    ...
    >>> bigram_tagger = nltk.BigramTagger(brown_news_train, cutoff=0)
...
tagger fails to tag the rest of the sentence. 
