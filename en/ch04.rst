...
in text.  We could ask to see the following words using the
``words()`` method:
...
Thus, `often`:lx: is typically followed by verbs, and never followed by nouns.
...
a list is expected, such as a ``for`` loop:
...
Let's first make our part-of-speech dictionary a bit more realistic and add some more words,
to create the situation where multiple keys have the same
value. The above method will not work in this case (why not?).  
Instead, we need to use our method of incrementally
...
Tagged corpora such as the Brown Corpus have been created manually, by
human experts following a set of guidelines on what tags to use for
what words. The amount of effort involved in manual tagging is usually
considerable, and not lightly undertaken. Can we build software to
automatically add part-of-speech tags to a text?
...
The rest of this chapter will review various approaches to automatic
part-of-speech tagging.  It can often be observed that downstream
processing |mdash| say syntactic parsing |mdash| is very sensitive to
the quality of the preceding part-of-speech tagging. (This is an
example of the "garbage-in, garbage-out" maxim.) Consequently, we will
consistently be concerned about the `performance`:dt: of a given
automatic tagging method. What do we mean by this? Simply, how well
does the automatic tagger reflect the judgments that would have been
made by a human annotator? Since we don't usually have access to an
expert and impartial human judge, we make do instead with `gold
standard`:dt: test data. This is a corpus which has been manually
...
.. note:: Of course, the humans who designed and carried out the
    original gold standard annotation were only human. Further
    reflection or analysis might lead to the view that the gold
    standard tag was actually a mistake. Nevertheless, the gold
    standard is by definition "correct" as far as the evaluation of an
    automatic tagger is concerned.


.. note:: The Default Tagger
------------------
...
Before we embark on a process to get 90+% accuracy, we need to do
