...
Let's look at which tags are the most common:
    ...
    >>> tag_fd = nltk.FreqDist(tag for (word, tag) in brown_news)
    >>> tag_fd.keys()
...
with the most frequent ones first:
    ...
    >>> list(nltk.FreqDist(a[1] for (a, b) in nltk.bigrams(brown_news) if b[1] == 'N'))
...
.. note::
   We found the relevant POS tags using ``tg.startswith('VB')``.  We could
   also have written ``tg[:2] == 'VB'``.  However, we think that the former
   choice is more readable.
