...
in speech synthesis.  For example, ``wind/NN``, as in
...
POS-concordance tool ``nltk.app.pos_concordance()``.  Use it
...
.. note:: |TRY|
   ...
   ``nltk.app.pos_concordance()``, or watch some of the *Schoolhouse Rock!*
...
Note that we'll be working with sentences from now on [say why?].
...
Now we can set up a tagger and use it to tag a sentence.
    ...
    [('Only', 'NN'), ('a', 'NN'), ('relative', 'NN'), ('handful', 'NN'), ('of', 'NN'),
    ...
    ('in', 'NN'), ('the', 'NN'), ('election', 'NN')]
    >>> regexp_tagger.evaluate(brown_news_tagged)
    0.20326391789486245
...
We can then use this information as the model for a "lookup tagger". 
    ...
    >>> baseline_tagger.evaluate(brown_news_tagged)
    0.45578495136941344
...
we train a unigram tagger, use it to tag a sentence, then evaluate:
    ...
    >>> unigram_tagger.evaluate(brown_news_tagged)
    0.8550331165343994
...
testing on the remaining 10%:
    ...
    >>> unigram_tagger.evaluate(brown_news_test)
    0.77294926741752212
...
tagger fails to tag the rest of the sentence.  Its overall accuracy score is very low: 
    ...
    >>> bigram_tagger.evaluate(brown_news_test)
    0.10276088906608193
...
.. pylisting:: sentence-tagging
   :caption: NGram Tagging at the Sentence Level
    ...
    size = int(len(brown_news_tagged_sents) * 0.9)
    brown_news_train_sents = brown_news_tagged_sents[:size]
    brown_news_test_sents = brown_news_tagged_sents[size:]

    t0 = nltk.DefaultTagger('NN')
    t1 = nltk.UnigramTagger(train_sents=brown_news_train_sents, backoff=t0)
    t2 = nltk.BigramTagger(train_sents=brown_news_train_sents, backoff=t1)
