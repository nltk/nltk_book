...
Python prompt, telling the interpreter to load material for chapter 1:
``from nltk.book.ch01 import *`` (this could take a few seconds).
This loads the text of the book *Moby Dick*
and we can ask the interpreter to give us some information about it, by
typing ``moby``:
...
That's a lot of words!  We can examine the contents of the book in a variety
...
`affection`:lx:, use: ``sensibility.concordance('affection')``.  Search the book of
*Genesis* to find out how long some people lived, using:
``genesis.concordance('lived')``.  You could look at ``inaugural``, the
...
We've also included ``chat``, the *NPS Chat Corpus*: search this for
...
and different texts.
    ...
    >>> sensibility.dispersion_plot(['Elinor', 'Marianne', 'Edward', 'Willoughby'])
...
followed by the "generate" function, e.g. ``genesis.generate()``:
...
Note that first time you run this, it is slow because it gathers statistics about word sequences.
Each time you run it, you will get different text.  Now try generating random text
in the style of an inaugural address or an internet chat room.
...
Explain the ``[x, y, z]`` notation.

Counting Vocabulary
-------------------

Apart from searching, we can count...

Next: tokens, types, nature of a text in Python as a list of tokens.
