...
Table freqdist_ summarizes the methods defined in frequency distributions.
...
gives the incorrect result (why?).
...
``[f(x) for y in z for x in y if g(x)]``.  Don't worry if you don't feel
