...
.. note::
   ...
   the ``book`` collection as shown in Figure nltk-downloader_.
...
.. figure:: ../images/nltk-downloader.png
   ...
   Downloading the NLTK Book Collection: This collection contains all
...
.. note::
   You need to have Python's Numpy and Pylab (aka Matplotlib) packages installed
...
followed by the "generate" method:
...
.. note:: Notice in the above example that we have split the definition
...
the frequency of individual words.  The ``collocations()`` method
...
``f`` is a function or method that operates on a word to compute its length, or to
