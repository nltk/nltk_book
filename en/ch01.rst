...
from set theory.  Suppose a predicate `P(w)`:math: is true just
in case the word `w`:math: is more than 10 characters long.  Here's
how we express the set of all such words that can be found in
the vocabulary `V`:math:.

.. _ex-set-comprehension:

.. ex:: {`w`:math: | `w`:math: |element| `V`:math: |wedge| `P(x)`:math:\ )

We can translate this expression into Python as follows:

    >>> [w for w in vocabulary if len(w) > 15]
...
The long words in ``text4`` reflect its national focus:
...
We expect such words to be hapaxes.  Instead, it would be interesting
to see frequently occurring long words, eliminating both the
frequent short words (e.g. `the`:lx:) and the infrequent long words
like (`antiphilosophists`:lx:).  Here are all words from the chat corpus
that are longer than 5 characters, that occur more than 5 times:

    >>> fdist5 = FreqDist(text5)
    >>> sorted(set(w for w in text5 if len(w) > 5 and fdist5[w] > 5))
...
This list gives a fair indication of the frequently-occuring content-bearing
words of the text.
...
.. exercise: produce a version of the text that leaves out the most frequent words
