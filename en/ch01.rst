...
|NLTK| includes the English WordNet, and it is
accessible to you in the variable ``wn`` (so long as you have already imported
...
Each synset is characterized by a set of synonymous words:
    ...
    >>> [lemma.name for lemma in wordnet.synset('dish.n.04').lemmas]
...
We can access semantically related forms.  The `hypernyms`:dt: of a word sense are more general terms... 
    ...
    >>> wn.synset('dish.n.02').hypernyms()
    [Synset('nutriment.n.01')]
    >>> wn.synset('dish.n.05').hypernyms()
    [Synset('directional_antenna.n.01')]
    >>>
