...
system to answer queries about train schedules in Europe. train0_
...
train0_. You don't want to end up with the
system trying to answer train1_ instead: 
...
isn't asking about the terminating station in train0_. But the
...
stations`:lx:, and the phrase `stop at`:lx: at the end train0_. The
required interpretation is made clearer in the "quiz question version
shown in train2_, where the question phrase fills the "gap" that is
implicit in train0_:
...
Consider the German questions germanagr_.
...
germanagra_ and germanagrb_ differ is more challenging. In this
...
Thus feat0_ is a `partial description` of a word; it lists some
...
context-free grammar.  We will begin with the simple CFG in agcfg0_.
...
|nopar| Example agcfg0_ allows us to generate the sentence `this dog runs`:lx:;
...
limited to individual productions. That is, within srule_, for example,
...
Production vprule_ can be thought of as saying that the `num`:feat: value of the
...
The grammar in listing feat0cfg_ illustrates most of the ideas we have introduced so
...
A final detail about feat0cfg_ is the statement ``%start S``.
...
tested and revised.  We have saved feat0cfg_ as a file named
...
lex_. (Note that we follow the convention that boolean
...
feature-based linguistic category, and ncat0_ as a convenient abbreviation.
...
#. |easy| Develop a variant of grammar feat0cfg_ that uses a
...
in dag02_ is the string "rue Pascal".
...
things become really interesting. First, let's define dag04_ in Python:
...
#. |soso| Consider the feature structures shown in Figure featstructures_.
...
This is illustrated in a modified portion of feat0cfg_, shown in subcatgpsg_.
...
book on the table`:lx:) might be represented as subcathpsg0_:
...
analyzing unbounded dependency constructions. slashcfg_ illustrates
...
The grammar in Figure slashcfg_ contains one gap-introduction production, namely
...
Using slashcfg_, we can parse the string `who do you claim that you
...
The grammar germancfg_ illustrates the interaction of agreement
...
#. |easy| Modify the German grammar in germancfg_ to incorporate the
   treatment of subcategorization presented in sec-subcat_. 

#. |soso| Extend the German grammar in germancfg_ so that it can
...
guides at ``http://nltk.org/doc/guides/featgram.html``,
``http://nltk.org/doc/guides/featstruct.html``, and
``http://nltk.org/doc/guides/grammartestsuites.html``.
