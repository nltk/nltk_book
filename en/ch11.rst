...
(|DRT|, [KampReyle]_) was developed with the specific goal of providing a means
for handling this and other semantic phenomena which seem to be
characteristic of discourse |mdash| sentences in a sequence whose
interpretation depends in part on what preceded them. A `discourse
representation structure`:dt: (|DRS|) presents the meaning of
discourse in terms of a list of  `discourse
referents`:dt: (the things under discussion in the discourse) together
with a list of `conditions`:dt: on those discourse referents. fig-drs1_
illustrates a |DRS| for the first sentence in drs0_.

.. _fig-drs1:
.. figure:: ../images/drs1.png
   :scale: 20

When the second sentence of drs0_ is processed, its content is merged
with what is already present in  fig-drs1_, leading to fig-drs2_.
...
and conditions as initialization parameters. These look respectively
like the individual variables and atomic open formulas of |FOL|. 

.. ex::
    DRS([x,y],[angus(x),dog(y),own(x,y)])

The easiest way to build a ``DRS`` object in |NLTK| is by parsing a
string representation of a |DRS|.
...
The discourse referents are interpreted like existentially-bound
variables, while the conditions are interpreted as though they are
conjoined. The ``toFol()`` method is a handy way of translating from a
|DRS| back to a formula of |FOL|.
