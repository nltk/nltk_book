...
the topmost discourse referents were interpreted like existentially quantifiers, while the conditions were interpreted as though they are
...
When we interpret a sentence, we do more than consider the bare truth
conditions of the sentence in isolation. We use a rich context for
interpretation, determined in part by the preceding context and in
part by our background assumptions. |NLTK| contains a simple discourse
processing module, ``nltk.inference.discourse``, which is inspired by
the CURT system presented in [Blackburn2005RIN]_.  A discourse is just
a sequence `S`:mathit:\ :sub:`1`, ... `S`:mathit:\ :sub:`n` of
sentences ; a `discourse thread`:lx: is a sequence `r`:mathit:\
:sub:`1`\ :sub:`i`, ... `S`:mathit:\ :sub:`n`\ :sub:`j` of readings,
one for each sentence in the discourse.
The module processes sentences incrementally
