...
Individual files, search engine results, RSS feeds: see method described in Section sec-accessing-text_.

Capturing the contents of a site, e.g. with ``wget``.

Customized web-crawlers, e.g. to have fine-grained control on where to look, which links to
follow, and where to store the results.  E.g. searching a bilingual news site to produce
a collection of parallel documents.  Modify an existing Python web crawler.

Many pitfalls: convert relative URL found inside an HTML document to an absolute URL
suitable for fetching; restricting the MIME types; avoiding getting trapped in cycles;
...
...   The main problem is that adequate tools
...
[text corpus standards, lexicon standards, ontologies, ...]
