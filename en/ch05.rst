...
.. TODO: cover tf.idf?
.. TODO: talk about sentiment detection?
...
.. _everything-is-classification:

----------------------------
Everything is Classification
----------------------------

In Section sec-lexical-resources_ we saw that male and female names have
some distinctive characteristics.  Names ending in `a`:lx:, `e`:lx: and
`i`:lx: are likely to be female, while names ending in `k`:lx:, `o`:lx:,
`r`:lx:, `s`:lx: and `t`:lx: are likely to be male.  Let's set this up
as a simple classification task.  First we need to prepare the training
and testing data:
    ...
    >>> train_names = namelist[500:]
    >>> test_names = namelist[:500]

The next step is to define a function to extract the features |mdash|
the observable properties |mdash| of the input that our classifier will exploit.
This function creates a dictionary containing features and values, in this
case a single feature ``last_letter`` whose value is the last letter of the
name:
...
Now we can process the ``namelist`` data, extracting the gender
features of each name, and use that to train a "naive Bayes" classifier.

    >>> features = [(gender_features(n), g) for (n,g) in namelist]
    >>> classifier = nltk.NaiveBayesClassifier.train(features[500:])
...
classified.  Although this science fiction movie is set in 2199,
it conforms with our expectations about names and genders.
Finally, we can systematically evaluate the classifier on
a much larger quantity of unseen data:

    >>> nltk.classify.accuracy(classifier, features[:500])
    0.774
...
suffixes are most informative.  Let's begin by defining a new feature function:
...
topic, genre, polarity, etc.  As we saw in Chapter chap-corpora_,
NLTK provides an interface to some of these corpora, and we will
briefly review the methods here.
...
In Section everything-is-classification_, we constructed a simple
name classifier, that tried to detect whether a name is more likely
as a female or male name.  It only used a single feature (the final
letter) and its performance was only 75%.  Let's use many more features
and see if we can improve the performance:
...
described in more detail in section classification-methods_.
...
[Discuss why its performance is worse; due to the naive Bayes' assumption
not holding.]
