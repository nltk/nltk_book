...
Language is full of patterns.  In Chapter chap-words_ we saw that
frequent use of the modal verb `will`:lx: is characteristic of news
text, and more generally, that we can use the frequency of a small
number of diagnostic words in order to automatically guess the genre
of a text (Table brown-types_).  In Chapter chap-tag_ we saw that
words ending in `-ed`:lx: tend to be past tense verbs, and more
...
#. How can various types of corpora be used to learn about linguistic
   patterns?
#. How can we use these patterns to create `models`:dt: that perform
   language processing tasks automatically?
#. What can we learn about language from these models?

In order to study and model linguistic patterns, we need to be able to
write programs to process large quantities of annotated text.  In this
chapter we will focus on data-intensive language processing, covering
manual approaches to exploring linguistic data in Section
sec-exploratory-data-analysis_ and automatic approaches in Section
sec-data-modeling_.

.. do we need more motivation and overview of this simple ontology
   (manual vs automatic)?
...
.. pylisting:: suffix-tagger

   def tag_features(word, n=3):
       features = {}
       for i in range(n):
           features["suffix(%d)" % n] = word[-(n+1):]
       return features

   def evaluate_suffix_tagger(classifier, tagged_data):
       size = int(len(tagged_data) * 0.9)
       train, test = tagged_data[:size], tagged_data[size:]
       train_features = [(tag_features(n), g) for (n,g) in train]
       test_features =  [(tag_features(n), g) for (n,g) in test]
       c = classifier.train(train_features)
       return nltk.classify.accuracy(c, test_features)
        
    >>> tagged_data = nltk.corpus.brown.tagged_words(categories='news')
    >>> evaluate_suffix_tagger(nltk.DecisionTreeClassifier, tagged_data)
    0.809864757359
...
conversations, instant messaging, and web pages.  Some of them
were described in Chapter chap-corpora_).
...
.. SB: This section could live later in the chapter.  It delays the presentation
   of concrete content.  If the concrete content came first, this could naturally
   follow afterwards, with the justification that some findings will be artifacts
   of the corpus that was chosen.
...
just an artifact of the particular corpus that was chosen.  For
...
.. SB: I think this section is redundant, since the section of ch03
   on useful applications of regular expressions covers similar ground;
   new content here would probably be better in ch03, unless it relates
   directly to the data modeling focus of this chapter.
...
.. SB: this is already partly covered in ch03's section about accessing
   text on the web.
...
.. SB: These are the first code examples in the chapter, and both
   depend on material from later chapters.  The tagged corpus
   example would be very helpful in the opening section of
   ch04, which is currently lacking an example of a linguistic
   analysis application of tagging.

But a more general solution is to simply write a short Python program
...
chunker," which can be used to make our program even simpler:
...
As a second example, the program shown in Listing sentential_complement_
...
.. SB: This is the main new content of this section that belongs in this chapter IMHO.
   However, I think it could stand alone (if the rest of this section goes into other
   chapters), perhaps near the start as a motivating example for the classification
   methods described in the chapter, or towards the end as another application area.
...
For example, if we wish to search for occurances of the "way
contruction," such as "pushed his way across the crowded room," we
...
able to find most of the occurances of the desired pattern.
...
* Exploratory data analysis, a technique for learning about a specific
  ...
  counting, and hypothesis refintement.

* Data modeling help us to understand the linguistic patterns, and
