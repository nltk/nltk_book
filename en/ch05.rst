...
not have a part-of-speech taggeed corpus that is sufficiently large to
...
`P(features, label) = P(label) |times| P(features|label)`:math:
...
.. _parameterized-naive-bayes:
   ...
   :math:`P(features, label) = w[label] |times| |prod|`\
       :sub:`f |in| features` :math:`w[f, label]`
...
.. _parameterized-maxent:
   ...
   :math:`P(input, label) = |prod|`\
       :sub:`joint-features(input,label)` :math:`w[joint-feature]`
...
distribution (i) because the labels' probabilities are evenly
