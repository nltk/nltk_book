...
contains, or its prefix.  However, we have no way to add features that
depend on the *context* that the word appears in.

In order to accomodate features that depend on a words context, we
...
Finally, we can use the corpus to train and test a classifier:
...
.. note:: *I plan to add an explicit example of error analysis here,
    based on one of the classifiers we've introduced in the first
