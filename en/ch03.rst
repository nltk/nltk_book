...
that appear in the same context, using NLTK's ``Text.similar()`` function:
    ...
    >>> text.similar('woman')
    man number world time end house state matter kind result day door
    group use way year rest part case problem
    >>> text.similar('bought')
    able made been used found was had , done expected given and followed
    that have seen put told said in
    >>> text.similar('over')
    in and of on to . , with at for but `` as is was from that when '' by
    >>> text.similar('the')
    a his this and in their an her that no its our however `` any these
    one ; my ?
...
words tagged with `VB`:gc:, `VBZ`:gc:, `VBG`:gc:, `VBD`:gc: or `VBN`:gc:.
    ...
    >>> brown_a = nltk.corpus.brown.tagged_words(categories='a')
    >>> fd = nltk.FreqDist(word + "/" + tag
                           for (word, tag) in brown_a if tag[:2] == 'VB')
    >>> fd.sorted()[:20]
...
.. pylisting:: findtags
   ...
   def findtags(tag_prefix, tagged_text):
       ...
       return dict((tag, cfd[tag].sorted()[:5]
...
frequent tag?  We can find out using a one-line program:
    ...
    >>> nltk.FreqDist(tag for (word, tag) in brown_a).max()
    'NN'
