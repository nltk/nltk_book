...
Consider the following text:
    ...
    >>> raw = """ARTHUR and BLACK KNIGHT: Aaah!, hiyaah!, etc.
...
[Compare the resulting ``tokens`` with a ``wordlist`` and report any tokens
that don't appear in the wordlist, using ``set(tokens).difference(wordlist)``\ ].
