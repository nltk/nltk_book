...
When we use a web search engine to find documents containing particular words,
we usually don't mind if word endings don't match exactly.  A query for
`laptops`:lx: finds documents containing `laptop`:lx: and vice versa.
In fact, we would say that `laptop`:lx: and `laptops`:lx: are two forms
of the *same* word.  This has many implications for our earlier word
counting activities.

There are various ways we can get at the stem of a word.  Here's a simple-minded
approach which just strips off something that looks like a suffix:

    >>> def stem(word):
    ...     if word.endswith('ing') or word.endswith('ous'):
    ...         return word[:-3]
    ...     elif word.endswith('ed') or word.endswith('ly') or word.endswith('es'):
    ...         return word[:-2]
...
to build up a disjunction of all the suffixes, 
...
See the split -- both parenthesised:

    >>> re.findall('^(.*)(ing|ly|ed|ious|ies|es|s)$', 'processing')
    [('process', 'ing')]

Greedyness problem, use non-greedy version of ``*``.

    >>> re.findall('^(.*?)(ing|ly|ed|ious|ies|es|s)$', 'processes')
    [('process', 'es')]

This works even when we allow empty suffix:

    >>> re.findall('^(.*?)(ing|ly|ed|ious|ies|es|s)?$', 'language')
    [('language', '')]

Use this on a wordlist.


Now a version which outputs all words (even those without an affix)
...
* just want the "stems":
    ...
    >>> [stem for word in wsj for stem in re.findall('^([a-z]*)(?:ing|ly|ed|ious|ies|s|es)$', word)]
    ['abandon', 'abat', 'abolish', 'abortion', 'abound', 'abridg', 'absolute',
    'absorb', 'academic', 'acced', 'accelerat', 'acceptanc', 'accept', 'accept',
    'acces', 'accessor', 'accidental', 'accident', 'accommodation', 'accomplishment',
    'accord', 'accountant', 'account', 'account', 'account', 'accru', 'accumulat', ...]
...
--------------------------------
Techniques for Normalizing Words
--------------------------------

So far, we have generally accessed texts with the help of |NLTK|\ 's
corpus readers, which have taken care of the work of dividing a text
file into tokens.  We've had to do some extra work to ignore
punctuation and case distinctions, using code like this::
