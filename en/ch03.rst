...
.. Note:: From this chapter onwards, our program samples will assume you
   ...
   statement: ``import nltk, re, pprint`` 
...
of ``+``, you might be able to guess what multiplication will do:
...
we have to precede it with another ``%`` to get it in the output.  We also have
to do something special to ensure that ``count/total`` does not use integer division
and give a result of zero (try replacing ``100.0`` with ``100`` to see what happens).

    >>> count, total = 3205, 9375 
    >>> "accuracy for %d words: %2.4f%%" % (total, 100.0 * count / total)
