...
Notice that `Project Gutenberg`:lx: appears as a collocation in this text.
...
A much of the text on the web is in the form of |HTML| documents.
...
with the ``nltk.data.find()`` method to access and read NLTK's corpus files
directly:
...
.. note:: |TRY|
   ...
   pattern ``p`` was matched, and ``nltk.draw.finding_nemo()`` which provides a graphical
