...
The WordNet Stemmer only removes affixes if the resulting word is in its dictionary
...
programming techniques that will be covered in Chapter REF). 
The WordNet Stemmer is a good choice if you want to compile the vocabulary
...
.. pylisting:: stemmer-indexing
   :caption: Indexing a Text Using a Stemmer
    ...
    class IndexedText(object):
        ...
        def concordance(self, word):
            ...
            for i in self._index[key]:
                target = '_' + self._text[i] + '_'
                print ' '.join(self._text[i-8:i] + [target] + self._text[i+1:i+8])
    ...
    king ! DENNIS : Listen , strange women _lying_ in ponds distributing swords is no basis
    a very brave retreat . ROBIN : All _lies_ ! MINSTREL : [ singing ] Bravest
    Nay . Come . Come . You may _lie_ here . Oh , but you are
    immediately ! No , no , please ! _Lie_ down . [ clap clap ] PIGLET
    is much danger , for beyond the cave _lies_ the Gorge of Eternal Peril , which
    Oh ... TIM : To the north there _lies_ a cave -- the cave of Caerbannog
    and lived ! Bones of full fifty men _lie_ strewn about its lair . So ,
    our fight ' til each one of you _lies_ dead , and the Holy Grail returns
...
Consider the following text:
    ...
    >>> raw = """ARTHUR and BLACK KNIGHT: Aaah!, hiyaah!, etc. [ARTHUR chops the BLACK KNIGHT's left arm off] 
...
of spaces, tabs, or newlines.
    ...
    >>> re.split(r'[ \t\n]+', raw)
    ['ARTHUR', 'and', 'BLACK', 'KNIGHT:', 'Aaah!,', 'hiyaah!,', 'etc.', '[ARTHUR',
    'chops', 'the', 'BLACK', "KNIGHT's", 'left', 'arm', 'off]', 'ARTHUR:', 'Now',
    'stand', 'aside,', 'worthy', 'adversary.', 'BLACK', 'KNIGHT:', "'Tis", 'but',
    'a', 'scratch.', 'ARTHUR:', 'A', 'scratch?', 'Your', "arm's", 'off!', 'BLACK',
    'KNIGHT:', 'No,', 'it', "isn't..."]
...
abbreviation, ``\s``, which means any whitespace character.  The above
...
Splitting on whitespace gives us tokens like ``[ARTHUR`` and ``scratch?``.
An alternative is to split on anything other than a word character:

    >>> re.split(r'\W+', raw)
    ['ARTHUR', 'and', 'BLACK', 'KNIGHT', 'Aaah', 'hiyaah', 'etc', 'ARTHUR', 'chops',
    'the', 'BLACK', 'KNIGHT', 's', 'left', 'arm', 'off', 'ARTHUR', 'Now', 'stand',
    'aside', 'worthy', 'adversary', 'BLACK', 'KNIGHT', 'Tis', 'but', 'a', 'scratch',
    'ARTHUR', 'A', 'scratch', 'Your', 'arm', 's', 'off', 'BLACK', 'KNIGHT', 'No',
    'it', 'isn', 't', '']

[Also gives us empty strings (explain why).]  We get the same result using
``re.findall(r'\w+', raw)``, using a pattern that matches the words instead of the spaces.
[``\w`` is complement of ``\W``]

    >>> re.findall(r'\w+|\S\w*', raw)
    ['ARTHUR', 'and', 'BLACK', 'KNIGHT', ':', 'Aaah', '!', ',', 'hiyaah', '!', ',',
    'etc', '.', '[ARTHUR', 'chops', 'the', 'BLACK', 'KNIGHT', "'s", 'left', 'arm',
    'off', ']', 'ARTHUR', ':', 'Now', 'stand', 'aside', ',', 'worthy', 'adversary',
    '.', 'BLACK', 'KNIGHT', ':', "'Tis", 'but', 'a', 'scratch', '.', 'ARTHUR', ':',
    'A', 'scratch', '?', 'Your', 'arm', "'s", 'off', '!', 'BLACK', 'KNIGHT', ':',
    'No', ',', 'it', 'isn', "'t", '.', '.', '.']
...
letters (e.g. `'Tis`:lx:, `'t`:lx:) but that sequences of two or more punctuation
characters are separated.  Unfortunately, we still have the token `[ARTHUR`:lx:.
We can add the open bracket as a special case.  For good measure we'll also create
a single token out of `...`:lx: and `BLACK KNIGHT`:lx:\ :

    >>> print re.findall(r"[A-Z]+\s+[A-Z]+|\[|\.+|\w+|\S\w*", raw)
    ['ARTHUR', 'and', 'BLACK KNIGHT', ':', 'Aaah', '!', ',', 'hiyaah', '!', ',',
    'etc', '.', '[', 'ARTHUR', 'chops', 'the', 'BLACK KNIGHT', "'s", 'left', 'arm',
    'off', ']', 'ARTHUR', ':', 'Now', 'stand', 'aside', ',', 'worthy', 'adversary',
    '.', 'BLACK KNIGHT', ':', "'Tis", 'but', 'a', 'scratch', '.', 'ARTHUR', ':',
...
None of these regular expressions has included parentheses for grouping sub-expressions.
Remember that ``re.findall`` interprets these specially, so they have to include the
...
So ``' '.join(saying)`` means: take all the items in ``saying`` and
concatenate them as one big string, using ``' '`` as a spacer between
the items.  (Many people find the notation for ``join()`` rather
unintuitive.)
