...
TODO: the other use: using the counts, not the results
...
[Python code for accessing results, extracting counts; mention Google API; Google 5-gram
corpus; replace above table with code that does the same thing.]
...
[first install the Universal Feed Parser from ``http://feedparser.org/``]
