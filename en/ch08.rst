...
stack (Section back-to-the-basics_); this is the `shift`:dt: operation.
...
as shown in chartinit_\ (a) for the initialized WFST and
...
finding a sequence `V NP PP`:gc: starting here.  This is known as an
`incomplete edge`:dt: (also a `self-loop`:dt: edge).
...
is telling us that we have complete edges for the lexical items.
...
records how far our goals have been satisfied. So in
chart_td_ex1_, we are predicting that we will be able to find an `np`:gc: and a
