...
module.  In Figure cfg1_ we define a grammar and show how parse a
...
The grammar in Figure cfg1_ contains productions involving various syntactic categories,
...
In order to develop simple grammars of your own, start off by
playing with the recursive descent
parser demo, ``nltk.app.rdparser.demo()``.
...
The demo comes with already loaded with a sample grammar; however, you
can edit this as you please. We will discuss the recursive descent
parsing algorithm in greater detail below, but for the time being you
can get an idea of how it works by using the *autostep* button.

If we parse the string `The dog saw a man in the park` using
the grammar shown in Figure cfg1_, we end up with two trees, similar to
...
recursion in the grammar in Figure cfg1_. However, the grammar in
Figure cfg2_ illustrates both kinds of
...
relations from heads to dependents.  Figure depgraph0_ illustrates a
...
The arcs in Figure depgraph0_ are labeled with the grammatical
...
The grammar in Figure cfg2_ correctly generates examples like
subcat1_.
...
.. note:: With a little imagination, it is possible to
...
In the tradition of dependency grammar, the verbs in subcat_ are said
...
the graphical demonstration ``nltk.app.rdparser.demo()``.
...
graphical demonstration ``nltk.app.srparser.demo()``.
...
`binary`:em: (see the discussion of normal forms in Appendix app-pcfg_).
...
#. |hard| Write a function that takes a grammar (such as the one defined in
   Figure cfg1_) and returns a random sentence generated by the grammar.
