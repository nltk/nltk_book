...
general case in theorem proving is to determine whether ``S |- g``
...
list of assumptions (which may be empty).
    ...
    False
    >>> inference.Prover9Command(P, [Q, NotQ]).prove()
    True
...
We invoke the Mace$ model builder by calling ``MaceCommand`` in an
...
inconsistent.
    ...
    >>> print MaceCommand(None, [a3, c1]).build_model()
    True
    >>> print MaceCommand(None, [a3, c2]).build_model()
    True
...
model in which the premises are true but the conclusion is false?
    ...
    >>> print MaceCommand(g, [a4, a5, a6]).build_model()
    True
