...
.. SB: simplifications:
   "might be considered as involving" -> "appears to require"
   "some amount of" -> "some"
   "We will start off by examining one in particular:" -> "Suppose that"
   "appropriate (and if we are lucky, correct)" (omit -- do we care
   about the distinction between appropriate and correct here?)

There are a variety of tasks that a computer could perform which might be
considered as involving some amount of natural language
understanding. We will start off by examining one in particular: we type in
...
way is extremely hard, but solving it for a particular case is
...
.. SB: probably better to point readers to the further reading section
   and to mention such resources there; also include an O'Reilly SQL book perhaps
...
.. SB: "The framework offered by the feature-based grammar formalism"
   -> "The feature-based grammar formalism"  (is it worth distinguishing
   a formalism and the framework it offers?)
...
.. SB: opening of next paragraph could be patronising.  "To summarize, ..."
   "to the extent that our software already understands SQL" -- why would
   it be a partial implementation of SQL?

Let's summarize what we've just learned. 
We have constructed a task that involves the computer returning useful data
...
.. SB: "linguistic ability" is ambiguous between ability to translate
   and ability to understand.  The former is the most likely interpretation
   for most readers, while the latter is probably the intended one.
...
her of your linguistic ability. We will return to this issue shortly.

.. SB: "the translation" is interpreted as referring to the translation
   activity just mentioned, not the interpretation of a sentence into SQL
...
.. SB: "something like SQL" -> "SQL" (I'm not sure what class of
   languages is intended by "something like SQL"
...
generic than something like SQL.
...
.. SB: I think the reference would be better in the further reading section.
...
.. SB: "in the second case" -- I think the order of the first
   and second cases has now been reversed.
...
particular situation.  In the second case, rather than translating a
...
.. SB: "which contribute to" -> "which guide" ?
...
.. SB: how about using M instead of Gamma for models?
...
.. SB: here t is used as a term; elsewhere it is a type
   "is effected in" -> "is implemented in"
...
.. SB: use either poodle or dog for the following example and its
...
.. SB: speaking of alphabetic variants, I still wonder if we can use
   our NLTK notation here.  What's the benefit of having a traditional
   notation and an NLTK notation?  The traditional notation only benefits
   the readers who already know it, and they would have no problem
   interpreting the NLTK notation.
...
.. SB: what does "The value itself" refer to?
...
.. SB: could the next two examples be combined, as subexamples
   of a single example?  This would break up the page less.
...
* Semantic Representations for English are constructed using a
...
* English sentences can be associated with a semantic representation by treating ``sem``
