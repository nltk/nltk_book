...
.. TODO:
   ...
   * check grammar involving +/-aux
   * s/rule/production/ in the grammars
   * BR: augment discussion of slash categories to show compatibility
     with ordinary phrases
   * BR: give example of parsing with German grammar
   * Give example of grammar with HPSG style subcat?
   * fix discussion of non-specified variables wrt code-feat0cfg
...
(using the notion of dependency discussed in chap-parse_), and
...
After tokenizing the input, we import the ``load_earley`` function load_earley1_
which takes a grammar filename as input and returns a chart parser
``cp`` load_earley2_.
Calling the parser's ``nbest_parse()`` method should yield one or more
parse trees.
...
present purposes. However, there is one implementation issue which
bears on our earlier discussion of grammar size. One possible approach
to parsing productions containing features is to compile out the
features so that we end up with a large, fully specified CFG along the
lines of ex-agcfg1_. By contrast, the parser process illustrated above
works directly with the underspecified productions given by the
grammar. 
...
.. _ex-sbartree:
.. ex::
      .. tree::  (S (NP you)(VP (V[-AUX,\ SUBCAT\ 2] claim)(SBar (Comp that) (S (NP you)(VP (V[-AUX,\ SUBCAT\ 1] like)(NP children))))))
...
The grammar in code-slashcfg_ contains one gap-introduction production, namely
...
daughter. Finally, ex-empty_ allows the slash information on `np` to
...
As you can see, the feature `objcase` is used to
specify the case that a verb governs on its object.
