...
``NP`` and ``V``. In place of atomic labels, we decompose them structures like
...
chap-data-intensive_ described how classification relies on detecting features of
...
(grammatical category) and ``ORTH`` (orthograpy, i.e., spelling). In addition, each
