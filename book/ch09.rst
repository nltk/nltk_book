...
values of `N`:gc: and `Det`:gc: are, they have to be the
same. Consequently, ``NP[NUM=?n] -> Det[NUM=?n] N[NUM=?n]`` will
permit ex-this_ and ex-dog_ to be combined into an `NP`:gc: as shown
...
prohibited because the roots of their constituent local trees differ
in their values for the ``NUM`` feature.
...
Production ``VP[NUM=?n] -> V[NUM=?n]`` can be thought of as saying
...
chap-parse_ that the Fundamental (or Completer) Rule in
...
edge. We will explain in more detail in sec-feat-comp_ how
