...
Example ex-agcfg0_ allows us to generate the sentence `this dog runs`:lx:;
...
Now ex-nprule_ says that whatever the ``NUM`` values of `N`:gc: and
...
of their
constituent local trees differ in their values for the ``NUM`` feature.
...
Production ex-vprule_ can be thought of as saying that the ``NUM`` value of the
head verb has to be the same as the ``NUM`` value of the `VP`:gc:
mother. Combined with ex-srule_, we derive the consequence that if the
``NUM`` value of the subject head noun is `pl`:fval:, then so is
the ``NUM`` value of the `VP`:gc:\ 's head verb.
...
category `Det`:gc:\ [``NUM``\ =\ `?n`:math:]. Why is this?  Well,
...
We can inspect the productions and the lexicon using the commands ``print
...
Observe that the parser works directly with
...
[PropN[``NUM`` = `sg`:fval:] |rarr| 'Kim', (0, 1)]. Recall from
...
in the edge 
[`np`:gc:\ [``NUM``\ =\ `sg`:fval:] |rarr| PropN[`num`:feat:\ =\ `sg`:fval:] |dot|, (0, 1)]
in code-featurecharttrace_,
where the feature ``NUM`` has been assigned the value `sg`:fval:.
...
``AUX``. Then our lexicon for verbs could include entries such as
...
In this case, we say that the feature ``AGR`` has a `complex`:dt: value.
...
This is similar to more conventional displays of AVMs, as shown in
ex-reentrant01_.
...
When we see a lexical category like ``V[SUBCAT=1]``, we can interpret the ``SUBCAT`` specification as a
pointer to the production in which ``V[SUBCAT=1]``
is introduced as the head daughter in a ``VP`` production.
By convention, there is a one-to-one correspondence between
``SUBCAT`` values and the productions that introduce lexical heads.
It's worth noting that the choice of integer which acts as a value for
``subcat`` is completely arbitrary |mdash| we could equally well
have chosen 3999, 113 and 57 as our three values in ex-subcatgpsg_.  On this
approach, ``subcat`` can *only* appear on lexical categories; it
makes no sense, for example, to specify a ``SUBCAT`` value on
``vp``.
...
As you will see, the feature `objcase` is used to
...
like /**b**/ might be decomposed into the structure [`+labial`,
`+voice`]. An important motivation was to capture
...
as [`num`\ =\ `masc`:fval:] is anomalous. Similarly, we cannot say
...
[`subcat`\ =\ `3`:fval:].  `Typed feature structures`:dt: were developed to
