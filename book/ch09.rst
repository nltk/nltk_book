...
confound the simple methods described in chap-parse_.  When nouns are marked
in some way |mdash| e.g. as plural, or feminine, or dative |mdash| other words
in the sentence may need to match.  If we ignore these long-distance constraints
in our formal grammars, they will produce the wrong syntactic structures leading
to incorrect interpretations.

The goal of this chapter is to answer the following questions: 

#. What are some important grammatical constructions that are difficult to analyze using our existing techniques?

#. How can we extend formal grammars so that grammatical properties can be shared between non-adjacent constituents?
...
We will also explore a technique for representing grammatical properties
using a type of dictionary called a "feature structure" and a computational
method for combining feature structures called "unification."

.. XXX need a new name for the following section;
   also I think it would be hard for newcomers to grasp
...
solution will probably also involve recognizing the syntactic structure of the
speaker's query.  In particular, your analyzer should recognize that
...
In this chapter, we will see a simple yet effective technique for keeping track of
long-distance dependencies in sentences.

.. XXX the following example is problematic since this agreement
   is not a purely local dependency
...
.. XXX the next paragraph seems inappropriate for setting the
   stage in this chapter because it says the agreement phenomenon
   is already covered in ch08, and the semantic constrast in
   the above sentence will be covered in ch10.
...
expressed within the |CFG|\ s that we presented in
...
dependencies such as number agreement. In chap-semantics_, we
...
sort that are appealed to in the context of machine learning
(chap-data-intensive_). Instead, we focus on a more restricted notion
of feature that plays a central role in building a certain class of
formal grammars called `feature based grammars`:dt:.  What's special
about these? The core ideas are probably already familiar to you. To
make things concrete, let's look at the word `these`:lx:. We'll be
modest, and assume that we do not know `everything`:em: about this
...
some attributes is exactly what you would expect from a partial
description!

.. 
 The framework of context-free grammars that we presented in
...
create feature structures in |NLTK|. In the final part of the chapter,
...
The following examples show pairs of strings, the first of which is
grammatical and the second not. (We use an asterisk at the start of a
string to signal that is ungrammatical.) 
...
.. pylisting:: code-featurecharttrace
   :caption: Trace of Feature based Chart Parser
    ...
    >>> from nltk.parse import load_earley # [_load_earley1]
    >>> cp = load_earley('grammars/book_grammars/feat0.fcfg', trace=2)  # [_load_earley2]
...
.. ex::
   ::
     ...
     V[SUBCAT=, TENSE=pres, NUM=sg] -> 'disappears' | 'walks'
...
which ``V[SUBCAT=trans]`` is introduced as the head daughter in a
...
placed as sisters of the head, whereas `adjuncts`:em: are
...
simply list a finite set of strings that separate the two? The answer
...
analyzing unbounded dependency constructions.  code-slashcfg_ illustrates
...
In developing grammars, excluding ungrammatical strings is often as
challenging than parsing grammatical ones. In order to get an idea
where and why a string fails to parse, setting the ``trace``
parameter of the ``load_earley()`` method can be crucial. Consider the
following parse failure:
    ...
    Completer |[-] . . .| [0:1] NP[AGR=[NUM='sg', PER=1], CASE='nom'] -> PRO[AGR=[NUM='sg', PER=1], CASE='nom'] *
    Completer |[-> . . .| [0:1] S[] -> NP[AGR=?a, CASE='nom'] * VP[AGR=?a] {?a: [NUM='sg', PER=1]}
    Scanner   |. [-] . .| [1:2] 'folge'
    Scanner   |. [-] . .| [1:2] TV[AGR=[NUM='sg', PER=1], OBJCASE='dat'] -> 'folge' *
    Completer |. [-> . .| [1:2] VP[AGR=?a] -> TV[AGR=?a, OBJCASE=?c] * NP[CASE=?c] {?a: [NUM='sg', PER=1], ?c: 'dat'}
    Scanner   |. . [-] .| [2:3] 'den'
    Scanner   |. . [-] .| [2:3] Det[AGR=[GND='masc', NUM='sg', PER=3], CASE='acc'] -> 'den' *
    Scanner   |. . [-] .| [2:3] Det[AGR=[NUM='pl', PER=3], CASE='dat'] -> 'den' *
    Completer |. . [-> .| [2:3] NP[AGR=?a, CASE=?c] -> Det[AGR=?a, CASE=?c] * N[AGR=?a, CASE=?c] {?a: [GND='masc', NUM='sg', PER=3], ?c: 'acc'}
    Completer |. . [-> .| [2:3] NP[AGR=?a, CASE=?c] -> Det[AGR=?a, CASE=?c] * N[AGR=?a, CASE=?c] {?a: [NUM='pl', PER=3], ?c: 'dat'}
