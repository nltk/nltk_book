...
Each file name has internal structure as shown in Figure fig-timit_.
...
organized into a tree structure, shown schematically in Figure fig-timit-structure_.
...
shown in Figure fig-datatypes_.  A lexical resource could be a conventional
...
Figure fig-datatypes_ is sufficient.
...
(see Section sec-lexical-resources_).
...
In cases where quality is paramount, the entire corpus
...
named-entity detection.  In Figure fig-windowdiff_ we see three
...
In the above example, the window had a size of 3.  We can increase or shrink this
window to fine-tune the windowdiff score.
...
in Figure fig-evolution_.  At a later date, should someone want to combine sources
...
and for committees of experts to revise and extend it corpus at periodic
...
results (see Section sec-accessing-text_).  However, in some cases we want to
obtain large quantities of web text.

The simplest approach is to obtain a published corpus of web text [list].
These have the advantage of being documented and stable.  Any experimentation
using such a corpus can be reproduced.

If the desired content is localized to a particular website, there are many
utilities for capturing all the accessible contents of a site [list].

Finally, for maximal flexibility and control, a web-crawler can be used [list; description].
These permit fine-grained control over where on where to look, which links to
follow, and how to organize the results.  For example, if we want to compile
...
For example, the program in Example code-html2csv_ strips out the HTML markup using ``ntlk.clean_html()``,
...
Converting Data Formats (notes)
-------------------------------

* structure of the conversion code usually follows that of the
  input (or the output)
* if the input and output are isomorphic we usually don't have
  to store anything
...
semantic content of every sentence.  These extra layers of annotation
...
* Word Tokenization: The orthographic form of text does not unambiguously
  identify its tokens.  A tokenized version, in addition to the conventional
  orthographic version, may be a very convenient resource.
...
in an endangered language.  However, this presupposes a transcription
system, since most languages lack a standard orthography.  If a language
...
simple interface to the user:  
    ...
    ...     else:
    ...         return []
    >>> spell_check('illefent')
    ['olefiant', 'elephant', 'oliphant', 'elephanta']
    >>> spell_check('ebsekwieous')
    ['obsequious']
    >>> spell_check('nucular')
    ['nuclear', 'nucellar', 'anicular', 'inocular', 'unocular', 'unicolor', 'uniocular']


[An extended reworking of the same task, this time for Rotokas.] 
...
For instance, in Example code-add-cv-field_ we define a function ``cv()`` which
...
Example code-add-cv-field_ shows what this does to a particular entry; note
...
In Example code-chunk-toolbox_ we set up
...
annotation types [annotation graphs, RDF, XML].  [Three-layer architecture.]

However, these have problems of their own.
More expressive formats allow the equivalent information to be expressed in
multiple ways, and make validation a much more complex process.
...
process must check rootedness and connectedness properties.
Developing annotation tools that work with these generic formats is
also problematic, e.g. a tree annotation tool could only load a subset
of the data that can be expressed in the format, so must do extra checking
[explain why XML validation technologies do not suffice here].

Another response has been to write one-off scripts (usually in Perl)
to manipulate corpus formats.  NLTK's corpus readers are a more systematic
...
[existing text corpus standards, lexicon standards, ontologies, ...]
...
[ACL software repository; NLTK repository for self-archived data being
...
#. |soso| In Example code-add-cv-field_ the new field appeared at the bottom of the
