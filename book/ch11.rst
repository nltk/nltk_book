...
For example, the program in Example html2csv_ strips out the HTML markup using ``ntlk.clean_html()``,
...
For instance, in Example add-cv-field_ we define a function ``cv()`` which
...
Example add-cv-field_ shows what this does to a particular entry; note
...
In Example chunk-toolbox_ we set up
...
#. |soso| In Example add-cv-field_ the new field appeared at the bottom of the
