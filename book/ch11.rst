...
annotation types.  However, these have problems of their own.
...
Notable attempts at general-purpose annotation formats,
such as *Universal Transcription Format* and the *Corpus Encoding Standard*,
have had little traction away from the institutions where they were developed. 
...
develop a common interface (cf ``nltk.corpus``).  For example,
there are many ways to store a phrase structure tree in a file.
We can use nested parentheses, nested XML elements,
a dependency notation with a (child-id, parent-id) pair on each line, and so on.
However, the logical tree structure in each case is the same, making it easy
to come up with a common interface.  This parallels some central themes
in computer science such as abstract data types, object oriented design,
...
and a common language (SQL), abstracting away from questions of file storage.
Innovations in storage can take place without requiring any change to
...
modeling problem, the data interchange problem, or the data validation problem.
We still have to work out how best to structure the data,
and write programs to read and write the format, and to validate the data.
