...
As we can see, XML is no panacea.  It provides us with a convenient format,
accompanied by an extensive collection of tools.
...
[Overview of Shakespeare XML Corpus]
    ...
    >>> xml = nltk.data.find('corpora/shakespeare/merchant.xml')
    >>> merchant = ElementTree().parse(xml)
    >>> merchant[0]
...
In code-chunk-toolbox_ we set up
