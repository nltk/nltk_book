...
program `Shoebox`:dt:, now over two decades old and re-released as `Toolbox`:dt:
...
.. figure:: ../images/windowdiff.png
   ...
   Three Segmentations of a Sequence of Characters or Tokens
...
follow, and how to organize the results.  For example, if we want to compile a
...
The program in code-html2csv_ strips out the HTML markup using ``ntlk.clean_html()``,
...
* sometimes have to build a structure one way, and read it off
  another way
* the hard case where no good, general solution is possible:
  round-tripping the data between two formats where information
  may be lost. 
...
create their own set of tools, which hinders those who
lack the necessary expertise, time or resources to make
their data accessible to others.
...
One response to this situation has been to forge ahead with generic formats
which are sufficiently expressive to capture a wide variety of
annotation types within a single model.  Please see sec-further-reading-data_ for pointers
to representative work in this area.  A challenge when working with
any generic format is that validation is a more complex process. 
For example, if the annotation task involves creating trees, and the
underlying notation permits arbitrary directed graphs, then the validation
process must check rootedness and connectedness properties required for trees.
Developing annotation tools that work with these generic formats is
also challenging, e.g. a tree annotation tool could only load a subset
of the data that can be expressed in the generic format.  Once modified
data was saved back to the file format, information about any non-tree
annotations may be lost.

Another response has been to write one-off scripts
to manipulate corpus formats.  The filespace of many members of the
|NLP| researchers are littered with such scripts, usually written in Perl.
NLTK's corpus readers are a more systematic
approach, founded on the premise that the work of parsing a corpus format
only needs to be done once (per programming language), then it can be shared. 
...
develop a common interface, e.g. ``nltk.corpus``.  Consider the
...
However, in each case the logical tree structure in is the same, making it easy
to devise a common interface.  This approach follows pervasive practice within
...
and a common language (SQL), to abstracting away from the idiosyncrasies
of file storage.  Innovations in storage can take place without requiring
end-user applications to be updated.  In the same way, a common corpus
interface insulates user programs from data formats.
...
However, most of the world's languages are threatened with extinction.
Many linguists are hard at work documenting the languages, constructing
...
part-of-speech taggers, parsers, named-entity recognizers, and so on,
...
On the face of it, it should be a straightforward matter to start collecting texts
in an endangered language.  Even if we ignore vexed issues such as who
owns the texts, and sensitivities within the texts such as secret cultural knowledge,
there is the obvious practical issue of transcription.
Most languages lack a standard orthography.  If a language
has no literary tradition, there will not be established spellings for
words, or conventions for punctuation.  Therefore it is usually necessary
...
a lexicon greatly helps this process, but it helps to have look-up methods
...
The first step is to identify confusable letter sequences,
...
.. ex::
   ::
     ...
     <entry><headword>whale</headword><pos>noun</pos><gloss>any of the larger cetacean
     mammals having a streamlined body and breathing through a blowhole on the head</gloss>
     <gloss>a very large person; impressive in size or qualities</gloss></entry>
...
is "verb", then the entry has a ``past_tense`` element to hold the
past tense of the verb).  To impose some order over all
...
However the flexibility comes at a price.  Each time we introduce a complication, e.g. by permitting
...
list of (word, sense) pairings.  This complex structure might be split
across three separate XML files.

As we can see, although XML provides us with a convenient format,
accompanied by an extensive collection of tools, it is no panacea.
...
Ignoring the entry of ``153`` for exchanges between people other than the top 5,
the largest values suggest that Othello and Portia have the most significant interactions.
...
chapters, such as counting, building frequency distributions, tabulating cooccurrences,
...
in a new way.  
Manually inspecting thousands of lexical entries is not practible.
...
uses the CFG format we saw in chap-parse_.
Such a grammar would model the implicit
nested structure of Toolbox entries, building a tree structure, where the
leaves of the tree are individual field names.  We could then iterate
...
"by hand" or gnerated automatically using software.
...
Searching OLAC Repositories
---------------------------

[Examples of searching for language resources, using LDC's OLAC search engine.
Searching for "German lexicon" finds:
...
Access XML record, e.g. ``http://www.language-archives.org/static-records/oai:sil.org:10218.xml``

NLTK Version 0.9 includes support for reading an OLAC record, for example:

    >>> path = nltk.data.find('corpora/treebank/olac.xml')
    >>> xml = open(path).read()
...
[NLTK repository for self-archived data being
set up at ``http://nltk.ldc.upenn.edu/``; best practices for describing
language resources ``http://www.language-archives.org/REC/bpr.html``]
...
Dictionary development process: ``http://www.sil.org/computing/ddp/``
...
Web-as-corpus resources, SIGWAC, CLEANEVAL

Annotation standards and XML: SIGANN.

XML book: many books... 

Linguistic Annotation Page: ``http://www.ldc.upenn.edu/annotation/``
...
Interannotator agreement [ArtsteinPoesio2008]_;
Windowdiff: [Pevzner2002Windowdiff]_.

Chapters on planning and constructing a corpus: [Meyer2002]_

Variety of topics in language engineering including corpus construction,
language on the Web: [Farghaly2003]_

Three layer architecture in linguistic annotation
[BirdLiberman2001]_

`General Ontology for Linguistic Description`:dt: (GOLD)
``http://www.linguistics-ontology.org/``

LaTeCH workshops on language technology for cultural heritage data.

Web crawling, chapter 3 of [Croft2009]_.
