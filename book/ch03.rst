...
``U+``), followed by their Unicode name.
    ...
    >>> for c in line:
    ...     if ord(c) > 127:
    ...         print('{!r} U+{:04x} {}'.format(c.encode('utf8'), ord(c), unicodedata.name(c)))
...
``{}`` in the format specifier of the code sample above, and replace
``c.encode('utf8')`` with ``c``, and if your system supports UTF-8,
...
* We can read text from a file ``f`` using ``text = open(f).read()``.
  We can read text from a URL ``url`` using ``text = request.urlopen(url).read().decode('utf8')``.
