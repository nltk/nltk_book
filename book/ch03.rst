...
.. XXX You use type() here but don't explain it until the section "The NLP Pipeline"
...
.. XXX "our first brush with reality" -- I see what you mean, but it
   seems a bit strong
...
.. XXX I feel these two commands: ``f = open('doc.txt')``, ``print
   f.read()`` are too important to be tucked away like this, ie inline
   in a note
...
.. XXX too many "can's" in the next two paras
...
Sometimes strings go over several lines.  Python provides us with a couple of
ways of entering them.  In the first method, a sequence of strings are
joined into a single string.  We need to use backslash or parentheses so that
the interpreter knows that the statement is not complete after the
first line:

.. XXX I was expecting you to follow "in the first method" with a 
...
.. XXX not clear what antecedent of "this" is

Unfortunately this method does not give us a newline between
...
.. XXX fig not referenced in text
...
the pipe symbol like this: |l|\ ``w(i|e|ai|oo)t``\ |r|\ , matching `wit`:lx:,
...
.. XXX say something about the use of | here, since it has
...
.. XXX in one of many senses of the term "word" --- actually, it seems
   to be what is meant by lexeme :-)
...
.. XXX Link this to hyponyms in WN discussion?
   There should be a reference in further reading to Hearst's original
   paper on this:
...
.. XXX given the context, I thought you were going to refer to
...
.. XXX say something about Lancaster stemmer. Is it meant to be a type
   of Porter Stemmer?
...
any space characters in the string split-space_; instead we need
to match any number of spaces, tabs, or newlines split-whitespace_:

.. XXX the point doesn't come across clearly unless the reader knows
   to look for the embedded \n
...
.. XXX some readers probably won't know what you mean by complement
.. here
...
.. XXX qualify this claim to make explicit that is only like findall()
   being used for tokenization!
...
[WORKING WITH A TEST SET TO IMPROVE A TOKENIZER.]
...
of information needed to reconstruct the source text as the basis for an objective function,
as shown in fig-brent_.

.. XXX explain what an objective function is
...
.. XXX it would be nice to have an explicit comparison of seg3 with
   seg1 and seg2
...
.. XXX wrap up with a comment about what this example shows
...
.. XXX we earlier drew a distinction between functions and methods --
   are we going to maintain it?

into strings.  To do this in Python we use the ``join()`` function, and specify
...
The ``join()`` function only works on a list of strings |mdash| what we have been calling a text
...
They start with the ``%`` character and end with conversion character
...
.. XXX mention 'for line in file' idiom
...
.. XXX no examples of re.sub() have been used in this chapter
...
Importance of reading Python library reference.
...
Collocations simple case of Multiword Expression
[Baldwin2009]_
...
Section 2.1 of [JurafskyMartin2008]_
...
[Friedl2002MRE]_ provides a comprehensive, in-depth study of regular
expressions, with examples from many programming languages, including Python.
...
#. |easy| 
   ...
   a) Use ``nltk.regexp_tokenize()`` to create a tokenizer that tokenizes
      ...
      regular expression, with clear setting out and inline comments,
...
#. |soso| Twy to write code to convert text into *hAck3r*, using regular expressions
...
#. |soso| Python's ``random`` module includes a function ``choice()`` which
   ...
   that produces a sequence of 500 randomly chosen letters, and put this
