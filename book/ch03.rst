...
As we saw in sec-texts-as-lists-of-words_ for lists, strings are indexed, starting from zero.
...
.. Note::
   ...
   in order to see the |nacute| displayed in your terminal.
...
If you replace
``c.encode('utf8')`` in unicode-info_ with ``c``, and if your system supports UTF-8,
...
that we first saw in _sec-overview-of-nltk.
...
In order to support efficient tokenization of large quantities of text,
|NLTK| includes an interface to the `Stanford Tokenizer`:dt:.
...
For example, in Chinese, the three-character string: |ai4|\ |guo2|\ |ren2|
(ai4 "love" (verb), guo2 "country", ren2 "person") could
be tokenized as |ai4|\ |guo2| / |ren2|\ , "country-loving person"
or as |ai4| / |guo2|\ |ren2|\ , "love country-person."
...
maintain.  Another solution is to use `string formatting`:dt:.
...
padded to width 6.  It is right-justified by default for numbers right-justified_,
but we can precede the width specifier with a ``'<'`` alignment option to make numbers left-justified left-justified_.
...
the ``'>'`` alignment option.
    >>> '{:6}'.format('dog') # [_left-justified-str]
    'dog   '
...
Recall from the listing in code-stemmer-indexing_ that we used a format string
...
#. |easy| Use a text editor to create a file
