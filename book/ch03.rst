...
When we index a string, we get one of its characters (or letters).  A single character is nothing special |mdash| its just
...
we haven't seen yet, is sown in tab-string-methods_.  For more information on strings, type
...
.. table:: tab-string-methods
   ...
   shown in tab-word-tests; all methods produce a new string or list
...
|mdash| as we will do from now on, you will avoid having to think about
these complications. 
...
and any whitespace between the angle brackets is ignored.

.. XXX this is our own syntax!  Explain this is specialized in NLTK.
...
programming techniques that are outside the scope of this book, and string formatting
techniques to be covered in sec-formatting_). 
...
[TODO: Explain how sentence segmentation followed by word tokenization can give
different results to word tokenization on its own.]

[TODO: add xref to discussion of sentence segmentation in chapter 6]
...
When we want to output these to a display or a file, we must convert these lists
...
the items.  (Many people find this notation for ``join()`` counter-intuitive.)
...
#. |soso| Write code to convert text into *hAck3r*, using regular expressions
