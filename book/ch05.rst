...
.. note::
   Regular expression taggers must be manually crafted.  sec-supervised-classification_
   describes a way to do such work automatically.
...
taggers using lists of tagged sentences, as shown in
code-sentence-tagging_.

.. pylisting:: code-sentence-tagging
...
.. pylisting:: code-brill-demo
   ...
   templates of the form ``X -> Y if the preceding word is Z``; the variables
   in these templates are instantiated to particular words and tags to create
   "rules"; the score for a rule is the number of broken examples it corrects
   minus the number of correct cases it breaks; apart from training a tagger,
   the demonstration displays residual errors.
...
#. |hard|
   ...
   exact tagging [Abney1996PST].  Explain why correct tagging of
...
#. |hard|
   Develop an n-gram backoff tagger that permits "anti-ngrams" such as
