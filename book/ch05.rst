...
#. How do we access the contents of corpora that have been "tagged" with parts-of-speech?
...
.. note:: |IMPORTANT|
   In this chapter, our program samples will assume you
...
Here we use |NLTK|\ 's built-in POS-tagger:

    >>> text = word_tokenize("And now for something completely different")
    >>> pos_tag(text, tagset='universal')
    [('And', 'CONJ'), ('now', 'ADV'), ('for', 'ADP'), ('something', 'NOUN'),
    ('completely', 'ADV'), ('different', 'ADJ')]

Here we see that `and`:lx: is ``CONJ``, a coordinating conjunction;
`now`:lx: and `completely`:lx: are ``ADV``, or adverbs;
`for`:lx: is ``ADP``, a preposition (strictly speaking, an adposition,
which is neutral as to whether it precedes or follows the noun);
`something`:lx: is ``NOUN``; and
`different`:lx: is ``ADJ``, an adjective.
These tags come from the "Universal Tagset", a set of 12 tags that has
been developed to facilitate comparison of syntactic annotations of
different languages (see tab-universal-tagset_).
...
Let's look at another example, this time including some homonyms:
    ...
    >>> pos_tag(text, tagset='universal')
    [('They', 'PRON'), ('refuse', 'VERB'), ('to', 'PRT'), ('permit', 'VERB'), ('us', 'PRON'),
    ('to', 'PRT'), ('obtain', 'VERB'), ('the', 'DET'), ('refuse', 'NOUN'), ('permit', 'NOUN')]

Notice that `refuse`:lx: and `permit`:lx: both appear as a verb and a noun.
...
Lexical categories may seem obscure to many readers. You might wonder what
justification there is for introducing this extra kind of information
in our language processing pipeline.
It turns out that many of these categories arise from superficial
analysis of the distribution of words in text.
Consider the following analysis involving
...
Notice that it uses different POS tags, e.g. ``at`` for article
(or determiner), and ``np-tl`` for a proper noun appearing in a title,
and so on.

Other corpora use different tagsets and file formats.
|NLTK|\ 's corpus readers provide a uniform interface so that you
don't have to be concerned with the different file formats.
For example, the corpus reader for the Brown Corpus loads the above
data into Python data structures as follows:
...
Here are some more examples, each with their "native" tagset:
...
.. note::
   To find out about the tagset used by a given corpus, you can
   consult it's ``readme()`` method.
   In addition, |NLTK| provides documentation for several tagsets,
   making it possible to look up the meaning of a tag or a
   set of tags specified using a regular expression,
   e.g. ``nltk.help.upenn_tagset('RB')``,
...
Now that we can read a tagged corpus, we are ready to do some
non-trivial processing. For example, we can find which POS tags
are the most frequent. Here we use the news section of the Brown Corpus:
...
.. note:: |TRY|
   ...
   What percentage of words are tagged using the first three tags of the above list? 
...
``(('The', 'DET'), ('Fulton', 'NOUN'))`` and  ``(('Fulton', 'NOUN'), ('County', 'NOUN'))``.
...
|NLTK| provides wrappers for other high-performance taggers,
implementing the same interface that we have seen for |NLTK|\ 's other
taggers above.

* Hidden Markov Model (HMM) Tagger: ``nltk.tag.HunposTagger`` (TnT Trigrams-n-tags tagger)
* Conditional Random Field (CRF) Tagger: ``nltk.tag.CRFTagger``
* Neural Network Tagger: ``nltk.tag.SennaTagger``
