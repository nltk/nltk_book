...
Perhaps we could have got that information by looking up a dictionary.
However, a tagger is smart enough to detect when a given word is
...
Notice that `refuse`:lx: and `permit`:lx: appear as both ``VBP``, a present tense verb,
and ``NN``, a noun.  Notice that each of these words is pronounced differently, depending
on its part of speech, e.g. `refUSE`:lx: is a verb meaning "deny," while `REFuse`:lx: is
a noun meaning "trash."  Evidently, for correct pronunciation of certain words, we need to know
the part of speech.  Try saying the above sentence with these words pronounced the wrong way,
to see just how incomprehensible the sentence becomes.  For this reason,
text-to-speech systems must perform POS-tagging in order to pronounce words correctly. 
...
their uses, but the details often seem obscure.  You might wonder what
...
For example, Figure fig-tag-indian_ shows data accessed using
``nltk.corpus.indian``.
...
(shown in Table tab-simplified-tagset_).
...
verb, as shown in Table tab-syntax-nouns_.
...
`eat`:lx: in Table tab-syntax-verbs_.
...
The program in Example code-findtags_ finds all tags starting with ``NN``,
...
We consider each three-word window in the sentence [three-word],
and check if they meet our criterion [verb-to-verb].  If the tags
match, we print the corresponding words [print-words]:
...
.. pylisting:: code-three-word-phrase
   :caption: Searching for Three-Word Phrases Using POS Tags
    ...
    def process(sentence):
        for (w1,t1), (w2,t2), (w3,t3) in nltk.trigrams(sentence) # [three-word]
        if (t1.startswith('V') and t2 == 'TO' and t3.startswith('V')): # [verb-to-verb]
            print w1, w2, w3 # [print-words]
...
Understanding why such words are tagged as they are in each context can help us clarify the distinctions between
the tags.
...
arbitrary types of information.  Table tab-linguistic-objects_ lists a variety
...
as we see from Table tab-linguistic-objects_, it has a much wider range of uses.
...
.. note::
   ...
   section sec-functions_.  This lambda expression specifies no
...
for tallying words shown in Figure fig-tally_.
...
The listing in Example dictionary_ illustrates an important idiom for
...
There's a second useful programming idiom at the beginning of Example
...
Now we can set up a tagger and use it to tag a sentence.
    ...
    [('``', 'NN'), ('Only', 'NN'), ('a', 'NN'), ('relative', 'NN'), ('handful', 'NN'), ('of', 'NN'),
    ('such', 'NN'), ('reports', 'NNS'), ('was', 'NNS'), ('received', 'VBD'), ("''", 'NN'),
    (',', 'NN'), ('the', 'NN'), ('jury', 'NN'), ('said', 'NN'), (',', 'NN'), ('``', 'NN'),
    ('considering', 'VBG'), ('the', 'NN'), ('widespread', 'NN'), ('interest', 'NN'), ...]
...
We can put all this together to write a simple (but slow) program to create and
evaluate lookup taggers having a range of sizes, as shown in Example baseline-tagger_.
...
we train a unigram tagger, use it to tag a sentence, then evaluate:
    ...
    ('of', 'IN'), ('the', 'AT'), ('terrace', 'NN'), ('type', 'NN'), (',', ','), ('being', 'BEG'),
    ('on', 'IN'), ('the', 'AT'), ('ground', 'NN'), ('floor', 'NN'), ('so', 'QL'), ('that', 'CS'),
...
.. Note::
   We specify the backoff tagger when the tagger is
...
[TODO: classification of unknown words based on string patterns.]
...
An n-gram tagger uses recent tags to guide the choice of tag for
the current word.  When tagging the first word of a sentence,
a trigram tagger will be using the part-of-speech tag of the
previous two tokens, which will normally be the last word of
the previous sentence and the sentence-ending punctuation.
However, the lexical category that closed the previous sentence
has no bearing on the one that begins the next sentence.

To deal with this situation, we can train, run, and evaluate
taggers using lists of tagged sentences, as shown in Example
sentence-tagging_.

.. pylisting:: sentence-tagging
   :caption: NGram Tagging at the Sentence Level
...
[TODO: How Brill tagger rules are learnt]

Using NLTK's Brill Tagger
-------------------------

Example brill-demo_ demonstrates |NLTK|\ 's Brill tagger...
...
as summarized in Table tab-morphosyntax_.
...
Other approaches to tagging involve machine learning methods (Chapter chap-data-intensive_.)
...
|NLTK|.  Each turn of the dialog is categorized as to its communicative
function::
...
#. |easy| Review the mappings in Table tab-linguistic-objects_.  Discuss any other
...
#. |soso| Example baseline-tagger_ plotted a curve showing
...
#. |soso|
   Modify the program in Example baseline-tagger_ to use a logarithmic scale on
