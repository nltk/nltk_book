...
entities that we might participate in interesting relations with one
another.  Typically, these will be noun phrases such as "The
Association of Computational Linguistics" or "The Knights Who Say Ni."
We can either restrict their attention to `proper nouns`:dt:, which
are typically easier to identify; or we can also consider pronouns and
common nouns, in which case we may need to do some work to determine
...
phrases.  For example, "the market for system-management software for
Digital's hardware" is a single noun phrase (containing two nested
...
NP-chunks are defined to be non-overlapping.  Generally, any
prepositional phrases or subordinate clauses that describe a noun
phrase will not be included in the corresponding NP-chunk, since they
might contain other noun phrases.
...
NP-chunker, we must first define a `grammar`:dt:, consisting of rules
...
.. pylisting:: code-chunkex
   ...
   >>> cp = nltk.RegexpParser("NP: ") # [_chunkex-cp]
...
the file representation of the information in fig-chunk-tagrep_::
...
.. pylisting:: code-unigram-chunkscore
    :caption:
Note that on lines unigram-chunkscore1_ and
unigram-chunkscore2_, we must convert the sentences from tagged
sequences into chunk trees in order to allow the ``ChunkScore`` object
to compare them.  
...
used in sequence-classification_ to build a part-of-speech tagger.
...
class, consec-pos-tagger_, is almost identical to the
``ConsecutivePosTagger`` class from code-consecutive-pos-taggger_.
...
.. pylisting:: code-classifier-chunker
   ...
   class ConsecutiveNPChunkTagger(nltk.TaggerI): # [_consec-pos-tagger]
...
classification task, and this requires training data (cf chap-data-intensive_).
The training data uses so-called IOB format, which we saw in
sec-chunking_.
...
|nopar| In this representation, there is one token
per line, each with its part-of-speech tag and its named entity tag.



.. XXX ADD CLASSIFIER BASED NE HERE!!
...
Other topics that use an IE approach:
anaphora resolution [Mitkov2002]_,
question answering [Pasca2003]_.
