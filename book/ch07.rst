...
.. XXX the brief mention/use of SQL seems odd here.  I added a python
   translation.  Should we just get rid of the SQL version?
...
.. XXX section: "Multiple Chunk Types" was here.  I moved it to
   ch07-extras because I didn't see that it added much, and it didn't
   feel very motivated.

.. XXX section" Chunking vs Parsing" was here.  I moved it to 
...
.. XXX these examples are confusing because there is the top-level
   'S' automatically supplied by the chunker, vs the 'S' supplied
   by the grammar.  Maybe change this second 'S' to 'CLAUSE' or
   something to make it more clear what's going on here?
...
.. XXX following text seems irrelevant / misleading
    Thus,
...
.. XXX do readers need to know that there ar two underlying chunkers?
...
as PERSON, ORGANIZATION, and GPE.
    ...
    >>> print nltk.ne_chunk(sent, binary=True) # doctest: +SKIP [_binary-ne]
    (S
      The/DT
      (NE U.S./NNP)
      is/VBZ
...
value of parameter ``relsym`` relsym_. The 

.. XXX what's the status of this example?  It doesn't show any output!
   Do we want to keep it? 

.. XXX now fixed, and tested with doctest
