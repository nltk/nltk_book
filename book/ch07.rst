...
In named entity detection, we search for and label mentions of
...
Like tokenization, which omits whitespace and sometimes omits punctuation,
chunking usually selects a subset of the tokens.
Also like tokenization, the pieces produced by a chunker do not overlap
in the source text.

The next section will explore chunking in some depth, beginning
...
sec-ner_ to the task of named entity recognition and relation extraction.
...
items in the input sequence (as already noted).
...
as illustrated in ex-parsing-chunking_:
...
backslash-escapes any non-word characters backslash-escape-ch7_,
...
.. pylisting:: code-chunker3
   ...
   def baseline_chunker(train):
       chunk_tags = [re.sub(r'(\W)', r'\\\1', tag) # [_backslash-escape-ch7]
...
Further challenges are posed by multi-part names like 
...
annotation but also part-of-speech tags. In principle, this allows us to devise
