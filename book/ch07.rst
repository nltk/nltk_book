...
translated as follows:
    ...
    >>> locs = [('Omnicom', 'New York'),
    ...         ('DDB Needham', 'New York'),     
    ...         ('Kaplan Thaler Group', 'New York'),
    ...         ('BBDO South', 'Atlanta'),
    ...         ('Georgia-Pacific', 'Atlanta')]
    >>> query = [e1 for (e1, e2) in locs if e2=='Atlanta']
    >>> print(query)
...
.. note:: Remember that our program samples assume you
   begin your interactive session or your program with: ``import nltk,
   re, pprint``

Let's quickly test our function.

    >>> from nltk.corpus import inaugural
    >>> doc = inaugural.raw('1789-Washington.txt') 
    >>> ie_preprocess(doc)
    [[('Fellow-Citizens', 'NNS'),
      ('of', 'IN'),
      ('the', 'DT'),
...
.. note:: |TRY|
   ...
   Use it to search the corpus for several other patterns, such as
   noun chunks with following verbal complements, e.g. ``"NOUN-COMP: {<JJ><N.*><IN><N.*><TO><V.*>}"``
...
.
    ...
    ...             "prevpos+pos": "{}+{}".format(prevpos, pos),  # [_chunk-fe-paired]
...
We noted in that it is possible to use the Stanford POS Tagger to in
situations where you want to tag a large quantity of text, and we also
gave instructions on how to download and install the tagger. A similar
approach is available for NER tagging, where we can interface to the
`Stanford NER Tagger`:dt:. The basic model provided
(``english.all.3class.distsim.crf.ser.gz``) can label PERSON,
ORGANIZATION, and LOCATION entity mentions. However, models are also
available that can distinguish up to seven classes of entity.

As before, the NER tagger can be downloaded from
<http://nlp.stanford.edu/software>_. You should ensure that the value
of the environment variable ``CLASSPATH`` include the pathname of the
folder that contains the ``stanford-ner.jar`` file, while
``STANFORD_MODELS`` should include the pathname of the folder that
contains the model files. The NER tagger can be run as follows:
...
value of parameter ``relsym`` relsym_. 
    ...
    ...         print(nltk.sem.clause(r, relsym="VAN")) # [_relsym]
    VAN("cornet_d'elzius", 'buitenlandse_handel')
    VAN('johan_rottiers', 'kardinaal_van_roey_instituut')
    VAN('annie_lennox', 'eurythmics')

.. note:: |TRY| Replace the last line relsym_, by
    ``print(rtuple(rel, lcon=True, rcon=True))``. This will show you
