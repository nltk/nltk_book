...
Recall that tokenizations `segments`:em: a sequence of characters into tokens,
...
form, such as the little example in Table tab-db-locations_, then
...
about ex-ie4_ to return the answers in Table tab-db-answers_?  This is
obviously a much harder task. Unlike Table tab-db-locations_, ex-ie4_
contains no structure that links organization names with
location names. As we will see in Chapter chap-semantics_, one
...
Table tab-db-locations_. Then we reap the benefits of powerful query
...
Information Extraction is usually broken down into at least two major steps:
`Named Entity Recognition`:dt: and `Relation Extraction`:dt:. Named
entities are noun phrases that denote
specific types of individuals such as organizations, persons, dates,
and so on. Thus, we might use the following |XML| annotations to
mark-up the named entities in ex-ie5_:
...
blindly runs into problems, as shown in Figure fig-locations_.
...
To summarize, we cannot reliably detect named entities simply
by looking them up in a gazetteer.
It is hard to develop rules that will correctly recognize
ambiguous named entities on the basis of their context.
Although lookup may contribute to a solution,
current approaches to named entity recognition usually treat it as a
classification task, and this requires training data (cf Chapter chap-data-intensive_).
The training data uses so-called IOB format, which we will
explore in Section sec-chunking_ below.
...
In this representation, there is one token
per line, each with its part-of-speech tag and its named entity tag.
...
.. 
 As shown above, the ``conll2002`` corpus contains not just named entity
...
as illustrated in Figure fig-chunk-segmentation_.  The smaller boxes show
...
the file representation of the information in Figure fig-chunk-tagrep_::
...
can be manipulated directly.  An example is shown in Figure fig-chunk-treerep_.
...
resulting chunk structure is returned.  Example code-chunker1_ shows a
...
In Section sec-tagged-corpora_ we saw how we could interrogate
...
remains.  These three possibilities are illustrated in Table tab-chinking-example_.
...
Example code-multistage-chunker_.  It has a stage for each of the chunk types.
...
stage of processing.
In Example code-chunker2_, two chunk patterns are applied to the input
...
have been using for our examples so far in this chapter (see Appendix
app-tagsets_ for full details). Because the Brown tagset
...
F-score (see Section sec-evaluation_).
...
of part-of-speech tags and chunk types.  Example cascaded-chunker_ has
...
The listing in Example traverse_ demonstrates this.  Note that
...
#. |hard|
   We saw in Chapter chap-tag_ that it is possible to establish
