...
We also define some tagged tokens to be chunked, and run the chunker on this input.
...
.. XXX are splitting/merging worth mentioning here?  It feels like we're
   spending a lot of real-estate describing something fairly nltk-specific,
   and a somewhat toy system at that.
...
.. XXX the source listing and doctest block in the following pylisting
   are getting their callout numbers assigened separately.  As a result,
   this turns out as gobbledygook.  I'll try to fix that in rst.py.
   But I find this a bit hard to follow, anyway.
...
.. XXX this uses nltk.chunk.conlltags2tree() which is not defined
...
types of NEs. These should be self-explanatory, except for "GPE", 
an abbreviation for geo-political entity e.g., city, state/province, or
country.
...
NLTK provides a classifier that is trained to recognize named entitie,
accessed with the function ``nltk.ne_chunk()``.  If we set the
parameter ``binary=True`` binary-ne, then named entities are just
...
.. XXX the following wants a normal citation.  (Would we ever say that a
   book was not freely available, but that you could buy it from XYZ publisher?)
...
patterns that are sensitive to these tags.
