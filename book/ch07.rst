...
At the start of this chapter, we pointed out that the first step in information
extraction is to identify phrases which described so-called named entities. Named
entities are associated with definite noun phrases that denote specific types
of individuals such as organizations, persons, dates, and so on. 

.. table:: tab-ne-types

  +----------------+-------------------------------------+
  |ORGANIZATION    |`Georgia-Pacific Corp.`:lx:, `WHO`   |
  +----------------+-------------------------------------+
  |PERSON          |`Eddy Bonte`:lx:                     |
  +----------------+-------------------------------------+
  |LOCATION        |`Melbourne`:lx:                      |
  +----------------+-------------------------------------+
  |DATE            |`175 million Candadian Dollars`:lx:  |
  +----------------+-------------------------------------+
  |TIME            |`Mar 2009`:lx:, `1:30 p.m.`:lx:      |
  +----------------+-------------------------------------+
  |MONEY           |                                     |
  +----------------+-------------------------------------+
  |PERCENT         |                                     |
