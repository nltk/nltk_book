...
answer down somewhere.  The amount of natural langauge text that is
available in electronic form is truely staggering, and is increasing
every day.  However, the complexity of natural language can make it
very difficult to access the information in that text.  
The state of the art in language processing technology is still very
far from being able to build general meaning representations from
unrestricted text.  

But if we instead focus our efforts on a limited set of questions or
relations, such as "where are different facilities located," or "who
is employed by what company," we can make significant progress.  The
...
.. XXX Add an "along the way.." type paragraph here?  What are we doing
   along the way?  NP chunking?
...
passage.  While it contains two occurences of `Washington`:lx:,
...
* Relation extraction can be performed using either rule-based
  ...
  text connecting entities and the intervening words; or using
  machine-learning systems, which typically attempt to learn
  these patterns automatically from a training corpus.
