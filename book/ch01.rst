...
One of the challenges of `Question Answering`:dt: as a research topic in
|NLP| is figuring out what knowledge can be drawn on to satisfy our
curiosity. Here are some examples of the kinds of question-answer
pairs that have been addressed by computational systems:
...
relational databases. An example of the latter is the ATIS database,
which included information about airlines, flights, fares, airports
and so on. Lots of useful information is stored as structured data
...
your questions in a special language such as SQL [ref]. Wouldn't it be
convenient if we could access the data by asking natural language
questions such as those in ex-qa1_:
...
of the world, and consequently the question answering systems which
...
engines, the prospect opened of retrieving answers to questions from
the masses of digital online text. This gave rise to so-called `Open
domain Question Answering`:dt:, which attempts to find answers in
large collections of documents, most notably web pages. Here's a rough
