...
.. TODO: introduce list.append() in ch01 as it's used in ch02
...
Dick* by entering ``text1`` followed by a period, the term
...
appears in?  We can find out as follows by appending the special term
``similar`` to the name of the text in question:
...
`very`:lx:.  The term ``common_contexts`` allows us to examine the
contexts that are shared by `monstrous`:lx: and `very`:lx:.
...
.. note::
   ...
   from the previous word.  While this is not correct formatting
   for English text, we do it to make it clear that punctuation does
...
group. When we count the number of tokens in a text, say the token
``his``, then we have to count every occurrence
of ``his`` in the text.
