...
the name of the text with the index inside brackets:

    >>> text4[173]
    'awaken'
    >>>

We can also find the index of the first occurrence of any word:
...
and can get back to some language analysis.
...
the interpreter to check if you got it right.  If you're not sure how
...
.. note::
   Notice in the above example that we split the definition
...
Don't worry if you don't feel confident with list comprehensions already,
...
Try the following, remember to include the colon and the four spaces:
...
The `Association for Computational Linguistics`:em: is the international organization that
