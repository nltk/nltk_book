...
If you have basic familiarity with both areas you can skip to Section
...
.. note:: |TRY|
   ...
   as you might expect |mdash| it does integer division when you type
...
.. note:: |IMPORTANT|
   ...
   there is no better way to learn about |NLP| than to dive in and try
   these yourself.  However, before continuing you need to install
   |NLTK|\ , downloadable for free from |NLTK-URL|.

   NLTK Data: Once you've installed |NLTK|, install the data required for the book by
   typing the following two commands at the Python prompt, then selecting
   the ``book`` collection as shown in Figure fig-nltk-downloader_.
...
.. figure:: ../images/nltk-downloader.png
   ...
   Downloading the NLTK Book Collection: See the available packages
   using ``nltk.download()``.  The book collection contains all
   ...
   The full collection of data is about five times this size (at the time
   of writing) and continuing to expand.
...
taking care to get spelling and punctuation exactly right.  You only need
to type the lines beginning with the ``>>>`` prompt:
...
.. note:: |TRY|
   Try seaching for other words; to save re-typing, you might be able to
...
(in an artificial text constructed by joining the addresses end-to-end).
...
in terms of the words and punctuation symbols that appear.  Let's use the
book of Genesis this time:
...
|mdash| such as ``can't`` or ``:)`` |mdash| that we want to treat as a group. 

How many distinct words does this text contain?
...
for the length of the set len-set_.  Although it has 44,764 tokens, this book
has only 2,789 distinct words, or "word types."
A `word type`:dt: is the unique form or spelling of the word, or the
word considered as a unique item of vocabulary.  Our count of 2,789
will include punctuation symbols, so we will generally call these
`types`:dt: instead of word types. 

Now, let's calculate a measure of the lexical
richness of the text and learn that each word is used 16 times on average
(we'll need to make sure Python uses floating point division):
...
we can come up with our own name for a task, like
"lexical_diversity" or "percentage", and associate it with a block of code.
Now we only have to type a short
...
defines a `function`:dt: called "lexical_diversity", and another
one called "percentage."
...
such functions when tabulating data, like Table tab-brown-types_.
...
the name of the text with the index inside brackets list-index1_.  We can
also find the index of the first occurrence of any word list-index2_:

    >>> text4[173] # [_list-index1]
    'awaken'
    >>> text4.index('awaken') # [_list-index2]
    173
    >>>
...
Thus, the slice ``15:18`` slice1_ includes ``sent`` elements at
indexes 15, 16, and 17, as we have verified.
...
e.g. putting ``sent[0]`` on the left of the equals sign list-assignment_.  We can also
replace an entire slice with new material slice-assignment_:

    >>> sent[0] = 'First' # [_list-assignment]
    >>> sent[19] = 'Last'
    >>> len(sent)
    20
    >>> sent[1:19] = ['Second', 'Third'] # [_slice-assignment]
    >>> sent
...
From the start of Section sec-computing-with-language-texts-and-words_, you have had
...
a variable name cannot be any of Python's reserved words, such as ``def``, ``if``,
...
Section sec-computing-with-language-texts-and-words_, and saw how to search for words
...
of a text.  As in Section sec-computing-with-language-texts-and-words_, you will try
...
.. note::
   Notice in the above example that we have split the definition
   of ``saying`` over two lines.  Python expressions can be split across
...
would be to keep a tally for each vocabulary item, like that shown in Figure fig-tally_.
...
The table in Figure fig-tally_ is known as a `frequency distribution`:dt:,
...
using ``fdist1.plot(50, cumulative=True)``, to produce the graph in Figure fig-fdist-moby_.
...
Neither frequent nor infrequent words help, so we need to try
something else.
...
that are longer than 7 characters, that occur more than 7 times:
...
authors, genres or languages.  Table tab-freqdist_ summarizes the functions defined in frequency distributions.
...
operators`:dt: are shown in Table tab-inequalities_.
...
using the functions listed in Table tab-word-tests_.
...
.. table:: tab-word-tests
   ...
   ``s.istitle()``        test if ``s`` is titlecased (all words have initial capital)
   ====================   ============================================================
...
In Section sec-computing-with-language-simple-statistics_, we saw some examples of
...
gives the wrong answer (why?).  Don't worry if you don't feel
confident with these already, since you'll see many more examples in the
following chapters.
...
a sense of the interesting challenges that are waiting for you.
...
(the thieves or the paintings).  Computational techniques for tackling this problem
...
Figure fig-sds_ shows the architecture of a simple dialogue system.
...
.. note:: |TRY|
   ...
   a conversation with a chatbot.  To see the available chatbots,
...
systems, but not enough data for machine learning techniques (a topic
we will cover in Chapter chap-data-intensive_).  Consequently, some
...
For a long time now, the holy grail of language understanding has been
`Machine Translation`:dt: (MT), which ultimately seeks to provide high-quality,
idiomatic translation of text between any pair of languages.
...
and with it, the genesis of Computational Linguistics itself.
...
a program that submits a sentence for translation into a
...
to German (line 1), but on the way back to English, this ends up as `Alice jump`:lx:
...
After this it quickly becomes nonsensical.
...
using superficial yet powerful counting techniques instead of
...
* A function is invoked by specifying its name followed by one or more
  parameters inside parentheses, like this: ``funct(x,y)``, e.g. ``len(text)``.

---------------
Further Reading
---------------

This chapter has introduced new concepts in programming and in natural language processing.
Many of them are consolidated in the following chapters.  However, you may also want to
consult the online materials provided with this chapter (at |NLTK-URL|), or read up on
some |NLP|\ -related concepts in Wikipedia (e.g. the Turing Test, the Type-token distinction).
...
Indurkya, Nitin et al (eds, 2009) *Handbook of Natural Language Processing*.  Marcel Dekker.
...
(second edition expected in 2010).
...
#. |easy| Given an alphabet of 26 letters, there are 26 to the power
   10, or ``26 ** 10`` ten-letter words we can form.  That works out
   to ``141167095653376L`` (the ``L`` at the end just indicates that
   this is Python's long-number format).  How many hundred-letter
   words are possible?

#. |easy| Review Section sec-a-closer-look-at-python-texts-as-lists-of-words_
   on lists and strings.  Define a string and assign it to a variable, e.g.
   ``my_string = 'My String'`` (but put something more interesting in the string).
   Try adding it to itself using ``my_string + my_string``, or multiplying
   it by a number, e.g. ``my_string * 3``.
...
#. |easy| Define a variable ``my_sent`` to be a list of words, using
   ...
   Use ``' '.join(my_sent)`` to convert this into a string.  Use ``split()``
...
#. |easy| Compare the lexical diversity scores for humor
   and romance fiction in Table tab-brown-types_.  Which genre is
...
#. |easy| Review the discussion of conditionals in Section sec-making-decisions_.
...
#. |soso| Use ``text9.index()`` to find the index of the word `sunset`:lx:.
   You'll need to insert this word as a parameter, between the parentheses.
...
#. |soso| Write the slice expression which extracts the last two words of ``text2``.
...
#. |soso| Review the discussion of looping with conditions in Section sec-making-decisions_.
