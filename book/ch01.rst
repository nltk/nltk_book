...
One of the challenges of `Question Answering`:dt: (QA) as a research topic in
|NLP| is figuring out what knowledge can be drawn on to satisfy our
curiosity. 

In the early days of Question Answering, the challenge was to use
natural language to query domain-specific knowledge stores, including
relational databases. Lots of useful information is stored as structured data
models, but it's hard for the ordinary user to access it. For example,
querying a relational database requires that you know how to formulate
your questions in a special language such as SQL. One database
Wouldn't it be
convenient if we could access the data by asking natural language
questions. One knowledge store that has been widely used as a domain
for spoken language dialogue systems is the ATIS database,
which includes information about airlines, flights, fares, airports
and so on. Systems that used this data were designed to answer questions
such as ex-qa1_:

.. _ex-qa1:
.. ex:: **Q**: How much is the cheapest flight from Boston to New York tomorrow?
   
        **A**: $257

Work on natural language interfaces to databases
has a long history, dating back to at least the 1970s, and flourished
particularly in the 1980s. fig-qa1_ illustrates the kind of
architecture used by such systems.

.. _fig-qa1:
.. figure:: ../images/qa1.png
   :scale: 80

   Generic Architecture for a natural language interface to a
   knowledge base: A question first undergoes syntactic and semantic
   analysis to produce a meaning representation. This in turn is
   transformed into a formal query of the kind required by the
   knowledge base and used to extract an answer, which is then
   returned to the user.


By design, traditional databases and other knowledge stores capture very restricted aspects
of the world, and consequently the QA systems which
interface to such backends tend to be highly domain-specific. With the
growth of the World Wide Web, and increasingly powerful search
engines, the prospect has opened up of retrieving answers to questions from
the masses of digital text available online. This has given rise to so-called `Open
domain Question Answering`:dt:, which attempts to find answers in
large collections of text documents, most notably web pages. As the
name suggests, Open domain QA should in principle be able to
deal with questions in any domain of knowledge which is covered by
the document collection.

Here's a rough sketch of how such approaches work:

1. Find the most informative words in a natural language question and
   turn them into a keyword query.

2. Input this query into a search engine to retrieve a set of documents from a document collection.

3. Identify snippets (i.e., short passage) of text in those documents which are potential answers.

4. Filter the snippets according to whether or not they contain
   information of the right type, given the question.

5. Return the top-ranked phrase or sentence after filtering according to some measure
   of appropriateness and relevance.

The filtering step (4) requires us to figure out what type of
information is being sought by the question, based usually on the
structure of the `wh`:lx: phrase, in particular on the `wh`:lx: "stem"
(`which`:lx:, `what`:lx:, `how much`:lx:, etc) and the head noun of
any following noun phrase,   For example, question type classification
should tell us that ex-qa1_ is looking for
a monetary amount. In the following examples, the
questions are looking for the name of a country and the name of a person;

.. _ex-qa2:
.. ex:: **Q**: Which country has the largest part of the Amazon rain forest?
   
         **A**: Brazil
	   
.. _ex-qa3:
.. ex:: **Q**: Who is the director of Dior?
   
        **A**: Raf Simons.

Although open domain QA tries to deal with information across all
domains, that doesn't mean that it is feasible to answer all kinds of
questions (e.g. *What is the meaning of life?*). The bulk of research
has focussed on `factoid questions`:dt: |MDASH| that is, fact-based
questions that expect a short answer |MDASH| together with `list questions`:dt: such as ex-qa4_, which
are looking for a number of instances that satisfy the query:

.. _ex-qa4:
.. ex:: **Q**: What airlines fly from Burbank to Denver?
   
        **A**: United, Southwest, American and US Airways

Harder to deal with than factoid and list questions are `why`:lx: questions, illustrated by ex-qa5_;
these require the computational system to detect causal relations
between sentences in text.

.. _ex-qa5:
.. ex:: **Q**: Why did World War 1 start?
   
        **A**: The immediate cause was the assassination of Franz Ferdinand, the \
	archduke of Austria-Hungary.


fig-qa2_ shows a high-level system archictecture of an open domain QA system.

.. _fig-qa2:
.. figure:: ../images/qa2.png
   :scale: 80

   Generic Architecture for an open domain QA system: A question
   first undergoes syntactic and semantic analysis to produce a
   meaning representation. This in turn is transformed into a keyword
   query to retrieve a set of candidate text documents from a document
   collection. Within each candidate document, the system identifies
   one or more snippets of text which might contain an answer. The
   snippets are filtered by matching against the question type,
   ranked, and then the best one is returned as the answer.

While traditional work on building interfaces to knowledge bases would
typically need to deal with the issue of connecting to a local storage
space, possibly via a database manager, the last decade has seen a
huge increase in the amount of structured knowledge which are
accessible over the web. Two examples which have been used for QA are
`DBpedia <http://wiki.dbpedia.org>`_ and `Freebase
<https://www.freebase.com>`_. These are both crowdsourced,
community-developed knowledge bases which need to accessed with formal
query languages. They combine factual knowledge (e.g., Douglas Adams
was a novelist) with conceptual knowledge (e.g., a novelist is a (kind
of) writer).
...
now to step back from the nitty-gritty of code in order to paint abigger picture of natural language processing.
...
it and `tokenize`:dt: it into a list of words and punctuation
...
You can convert a list of tokens into an |NLTK| "Text" object.
This makes it possible to perform the simple text processing operations described in the following sections:
...
There are many ways to examine the context of a text apart from just reading it.
For example, a concordance shows us each occurrence of a particular word, in context.
...
We just saw that a concordance permits us to see words in context. For example,
...
of contexts? Let's search for some "textually-similar" words.
...
``len(w)`` is greater than 15; all other words will be ignored.
...
look at the distribution of word lengths in a text, by creating a frequency distribution
out of a list of numbers, where each number represents the length of a word in the corresponding
text: 
...
As we saw above, |NLTK| supports the conversion of unbroken text into lists of tokens,
sequences of words and punctuation. The simplest way is to use the built-in ``word_tokenize``
function. We'll try to tokenize something that is a little challenging,
from *Alice's Adventures in Wonderland*:
    ...
    'wo', "n't", 'have', 'any', 'pepper', 'in', 'my', 'kitchen', 'AT', 'ALL', '.', 'Soup',
    'does', 'very', 'well', 'without', '--', 'Maybe', 'it', "'s", 'always', 'pepper',
    'that', 'makes', 'people', 'hot-tempered', ',', "''", '...']

Note the deliberate splitting of contractions into two words (e.g. for `I'm`:lx: and `it's`:lx:),
making it a small further step to normalize the tokens (e.g. `'m`:lx: becomes `am`:lx:).
See chap_words_ for more details about tokenization.

|NLTK| includes several "Part-of-speech taggers"
which automatically associate lexical categories with words, for example:

    >>> from nltk import pos_tag
    >>> pos_tag(tokens)
    [('``', '``'), ('When', 'WRB'), ('I', 'PRP'), ("'M", 'VBP'), ('a', 'DT'), ('Duchess', 'NNP'),
    (',', ','), ("''", "''"), ('she', 'PRP'), ('said', 'VBD'), ('to', 'TO'), ('herself', 'PRP'), ...]

Each of the capitalized labels represents a part of speech tag, such
as wh-adverb, preposition, verb in present tense, determiner, and so
on. See chap_tag_ for more details about tagging.


* parsers

* interfaces to other parsers: Stanford Parser, Malt Parser, BLLIP parser
...
.. Charles T. Hemphill, John J. Godfrey, and George
.. R. Doddington. 1990. The ATIS spoken language systems pilot
.. corpus. In Proceedings of the workshop on Speech and Natural
.. Language (HLT '90). Association for Computational Linguistics,
.. Stroudsburg, PA, USA, 96-101. DOI=10.3115/116580.116613
.. http://dx.doi.org/10.3115/116580.116613
