...
.. XXX can we get rid of "special" in the following para?
...
by appending the special term ``similar`` to the name of the text in
...
Austen use this word quite differently from Melville; for her, `monstrous`:lx: has
...
US presedential inaugural addresses end-to-end).
...
.. note::
   ...
   more about these issues in chap-words_.
...
term ``len`` to get the length of something and we'll apply it to the
book of Genesis:
...
task for us is called `function`:dt:, and 
...
In the definition of ``lexical diversity()`` fun-parameter1_, we
...
To recap, we use or `call`:dt: a function like ``lexical_diversity()`` by typing its name, followed
by an open parenthesis, the name of the text, then a close
...
Later we'll see how to use functions when tabulating data, like tab-brown-types_.
...
.. XXX it's a little confusing that we assign a value to sent1 here,
   when it's already recieved on from the "from nltk.book import *"
...
.. XXX Concatentation seems like a much more useful operation than 
   multiplication; perhaps just mention concatenation, or at least
   give it more focus?

A pleasant surprise is that we can use some of Python's arithmetic operators on lists.
...
by everything from the second list:
    ...
    >>> ['Monty', 'Python'] * 2 # [_list-times-number]
...
names that have lists as values.
...
represents this position is the item's `index`:dt: and we instruct Python
...
or |mdash| more generally |mdash| the elements of any list.
...
.. XXX in the previous example, it may not be clear why sorted(noun_phrase)
   gets the value that it does; we mentioned in passing that upper case
   words get alphabatized before lower case ones, but they may not remember
   that here.
...
.. XXX this example could use some callouts.
...
since it tells us how the total number of word tokens in the text
...
first fifty of these by slicing the list freq-dist-slice_.
...
only, the so-called `hapaxes`:dt:?  View them by calling ``fdist1.hapaxes()``.
...
.. XXX rather than abstracting out property P, how about just comparing:
     {w | w \in V & len(w)>15}
   with:
     [w for w in V if len(w)>15]
   ?
   Also, note that the comparison to sets is a little misleading, since
   this is a *list* comprehension, not a set comprehension.  If V 
   weren't a set, then the results of the math & python would be
   different.

.. XXX resolution: say something about how these two things are different
   (NB token / type distinction made elsewhere)
...
A `collocation`:dt: is a sequence of words which occur together
unusually often. Thus `red wine`:lx: is a collocation, while `the
wine`:lx: is not. A characteristic of collocations is that they are
resistant to substitution with words that have similar senses |mdash|
`maroon wine`:lx: sounds definitely odd.  

To get a handle on collocations, we start off by extractng from a text
...
word token in the text.  But there are only twenty distinct
...
words of length 3 account for roughly 50,000 (20%) of the words of the
...
.. XXX see: http://code.google.com/p/nltk/issues/detail?id=320
   Get rid of the fdist1<fdist2 line?
...
Python "test" which yields either true or false.
In the cases above,  the condition is always a numerical comparison.
...
.. XXX I'm not sure we want to call this *the* complete pipeline for
   NLP.  Maybe just delete that sentence and go streight to the 
   next one; and join that with the following paragraph.
...
* A function is a block of code which has been assigned a name and can
