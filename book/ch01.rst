...
name suggests, such approachs should in principle be able to find
answers to questions in any domain of knowledge which is covered by
the document collection.
Here's a rough
sketch of how such approaches work:
...
one of more names of airlines. In the following examples, the
...
These are sometimes called `factoid questions`:dt:\ : that is, fact-based
questions that expect a short answer, and much of the research in QA
has focussed on these or on `list questions`:dt such as ex-qa4_, which
...
Harder to deal with are `why`:lx: questions, illustrated by ex-qa4_;
