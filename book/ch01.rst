...
.. note:: |TRY|
   ...
   bracketing expressions.  Note that in Python 2, division doesn't always behave
...
then read the explanation that follows.
    ...
    >>> vocabulary1 = list(fdist1.keys()) # [_freq-dist-keys]
...
.. table:: tab-freqdist
   ...
   ``list(fdist.keys())``           the samples sorted in order of decreasing frequency
