...
Once you've installed |NLTK|, install the data required for the book by
...
the text of several books (this will take a few seconds).  Here's what
you will see.  Take care to get spelling and punctation right, and
...
.. note:: |TRY|
   ...
   *US Presidential Inaugural Addresses* to see examples of English going
...
positive connotations, and sometimes function as an intensifier like the word
...
When you do this, many screens of words will fly past.  Now try the following:
...
defined a `function`:dt: called "lexical_diversity", and another
...
such functions when tabulating data, like tab-brown-types_.
...
We can inspect it by typing the name inspect-var_, and we can ask for its length len-sent_.
...
We can also use arithmetic operators with lists in Python.
...
the elements of a list by their number |mdash| or `index`:dt: |mdash| by following
...
is the first word, ``'word1'``, while ``sent`` element 9 is ``'word10'``. 
...
Here we verify that the slice ``5:8`` slice1_ includes ``sent`` elements at
indexes 5, 6, and 7:
...
Below we put ``sent[0]`` on the left of the equals sign list-assignment_.  We can also
...
who reads your Python code |mdash| what your code is meant to do.
...
.. caution::
   ...
   would be distinct variables.  Variable names cannot contain whitespace,
   but you can separate words using an underscore, e.g. ``my_var``.
...
of a text.  As in sec-computing-with-language-texts-and-words_, you will try
...
authors, genres or languages.  tab-freqdist_ summarizes the functions defined in frequency distributions.
...
to sites that demonstrate language technologies.  You may also like to read up on
...
Indurkya, Nitin and Fred Damerau (eds, 2009) *Handbook of Natural Language Processing*
(Second Edition) Chapman & Hall/CRC. 2009.  [Dale00handbook]_
...
#. |easy| Review sec-a-closer-look-at-python-texts-as-lists-of-words_
   ...
   b) Try adding it to itself using ``my_string + my_string``, or multiplying
