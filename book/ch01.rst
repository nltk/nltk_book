...
simply check that you are running Python 2.6 or 2.7, or Python 3.2+
(here it is for 3.4.1):
...
.. note:: |TRY|
   ...
   bracketing expressions.  Note that in Python 2, the ``/`` operator rounds
   fractional results downwards (so ``1/3`` will give you ``0``).
   In order to get the expected behavior of division in Python 2,
...
XS    Building word-context index...
...
.. table:: tab-freqdist
   ...
   ``fdist.most_common(n)``         the ``n`` most common samples sorted in order of decreasing frequency
...
.. note::
   The next example does not work any more. Please try something
   similar using ``translationparty.com``.
...
(which is automatically loaded when you import this chapter's
