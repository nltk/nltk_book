...
.. note::
   ...
   In sec-overview-of-nltk_, we showed how you
   ...
   tasks from sec-overview-of-nltk_:
...
In sec-overview-of-nltk_, we looked at
...
We introduced frequency distributions in sec-overview-of-nltk_.
...
like the one we saw in sec-overview-of-nltk_
...
We look up a dictionary by giving its name followed by a `key`:dt:
...
Unlike the word `motorcar`:lx:, which is unambiguous and has one
synset, the word `car`:lx: is ambiguous, having five synsets:
...
A good tool for creating annotated text corpora is called `Brat`:em:,
...
#. |soso| Write a program to create a table of word frequencies by genre,
   like the one given in sec-extracting-text-from-corpora_ for modals.
