...
.. pylisting:: random-text
   ...
   from the text of the book of Genesis, then constructs a conditional
   frequency distribution to record which which words are most likely
   to follow a given word; e.g. after the word `living`:lx:, the most likely
   word is `creature`:lx:; the ``generate_model()`` function uses this
...
.. pylisting:: plural
   ...
   plural form of any English noun; the keyword ``def`` (define) is
   followed by the function name, then a parameter inside parentheses,
   and a colon; the body of the function is the indented block of
   code; this code tries to recognize patterns within the word and
   process the word accordingly; e.g. if the word ends with `y`:lx:,
   delete the `y`:lx: and add `ies`:lx:.
...
.. note::
   ...
   (e.g. ``word`` in Figure plural_).  To call such functions, we give
...
.. pylisting:: unusual
   ...
   then removes all items that occur in an existing wordlist, leaving just
   the uncommon or mis-spelt words.
