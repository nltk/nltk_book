...
Table tab-brown-sources_ gives an example of each genre
...
.. note::
   Many other English text corpora are provided with |NLTK|.  For a
...
summarized in Figure fig-text-corpus-structure_.
...
be extended to work with new corpora.  Table tab-corpus_ lists functionality
...
In Example code-random-text_, we treat each word as a condition, and for each one
...
.. pylisting:: code-random-text
   :caption: Generating Random Text: this program obtains all bigrams
             ...
             conditional frequency distribution to record which which
...
.. note:: |IMPORTANT|
   ...
   than using the interpreter (e.g. see Example code-random-text_ above).  To save typing,
   you can access this code via the |NLTK| website.  The listing in Example code-random-text_
...
function.  The function ``plural()`` in Example code-plural_
...
.. note::
   ...
   (e.g. ``word`` in Example code-plural_).  To call such functions, we give
...
words in a text corpus, as shown in Example unusual_.
...
hierarchy is illustrated in Figure fig-wn-hierarchy_.
...
#. |soso| Write a program to generate a table of lexical diversity scores (i.e. token/type ratios), as we saw in
   Table tab-brown-types_.  Include the full set of Brown Corpus genres (``nltk.corpus.brown.categories()``).
   Which genre has the lowest diversity (greatest number of tokens per type)?
   Is this what you would have expected?

#. |soso| Modify the text generation program in Example code-random-text_ further, to
...
#. |hard| Modify the ``generate_model()`` function in Example code-random-text_ to use Python's
