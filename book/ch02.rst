...
In Example random-text_, we treat each word as a condition, and for each one
...
.. note:: |IMPORTANT|
   ...
   than using the interpreter (e.g. see Example random-text_ above).  To save typing,
   you can access this code via the |NLTK| website.  The listing in Example random-text_
...
function.  The function ``plural()`` in Example plural_
...
.. note::
   ...
   (e.g. ``word`` in Example plural_).  To call such functions, we give
...
#. |soso| Modify the text generation program in Example random-text_ further, to
...
#. |hard| Modify the ``generate_model()`` function in Example random-text_ to use Python's
