...
electronic text archive containing
...
.. note::
   ...
   ``nltk.corpus``, as in the above example, you have to employ the
...
it has a recurrent value of `4`:math:.  (In fact, the average word length is really
...
.. XXX We could make the difference between the different access
   methods emerge more sharply by doing a direct comparison, along the
   following lines:
...
The above example also showed how we can access the "raw" text of the book raw-access_,
...
.. XXX And was the notion of word offset ever explained?
...
.. XXX Are Movie Reviews and Sentiment Polarity the same corpus?
   XXX Why not list all the corpora currently shown on the NLTK web page?
   XXX The notation "(sel)" isn't very transparent, and anyway should be explained. 
   XXX Mention that Floresta is parsed (and tagged?)
...
``corpus_root`` corpus-root_.

    >>> from nltk.corpus import PlaintextCorpusReader
    >>> corpus_root = '/usr/share/dict' # [_corpus-root]
...
.. XXX Give another example, showing how to use e.g. the Penn treebank reader on your
   own, full version of the treebank. Assume the reader is using a Windows filesystem!
...
.. XXX Remind reader what this table means
...
.. XXX Table not referenced in text
...
.. XXX This would make more sense if it followed discussion of modules below.
...
.. XXX We haven't yet explained about using help, or given any examples
   This should probably have been in ch01
...
.. XXX xref to brief discussion of functions in ch01
...
task.  It is usually defined to take some inputs, using special variables known as `parameters`:dt:,
...
.. XXX the distinction between functions and methods is picked up in the chapter
   summary. If it's that important, then maybe it shouldn't be tucked away in a note.
...
.. XXX It would be nice to unpack this in the text. Not sure about lexeme = lex entry.
...
and length constraint length-constraint_ (and we'll
...
supplied letters, especially since some of the latter appear twice (here, the letter
`v`:lx:).

.. XXX Expand this?

We use the ``FreqDist`` comparison method freqdist-compare_ to check that the frequency of each
...
.. XXX This segment sequence was already discussed in ch01 :-{
...
.. XXX remind reader what [-1] means
...
.. XXX This suggests that the reader might have been surprised not to see "internal
   structure" in wordlists, but the converse is probably true.
...
.. XXX is this the first time we have seen double for expressions in comprehensions?
   Is there a case for introducing it more gently, perhaps comparing with explicit
   for loops
...
.. XXX first line is going to be v. indigestible for many readers
...
.. XXX I wonder about dropping this -- ie just wait till ch11?
...
.. XXX refer back to our tree diagram?
...
The WordNet package includes a variety of similarity measures
...
.. XXX Seems a bit curt. Explain help()?
...
Phonetics: chapter 7 of [JurafskyMartin2008]_.

Lexical semantics incl measures of word similarity: chapter 20 of [JurafskyMartin2008]_.
...
.. XXX "access is more specific" -- unclear
