...
#. |easy| Create a classifier to perform word sense disambiguation, using
   the ``senseval`` corpus.

#. |easy|
   Compare the performance of different machine learning
   methods on each of the different tasks that were presented in
   the chapter.
...
#. |hard|
   Using the prepositional phrase attachment corpus (``ppattach``), 
   build a classifier that predicts what preposition should be used
   to combine two given nouns.
