...
.. XXX last two paras: is it "simple-typed values" or "simple value types"
...
found most useful in distinguishing the names' genders:
    ...
    Most Informative Features
                 last_letter = 'a'              femal : male  =     38.3 : 1.0
                 last_letter = 'k'               male : femal =     31.4 : 1.0
                 last_letter = 'f'               male : femal =     15.3 : 1.0
                 last_letter = 'p'               male : femal =     10.6 : 1.0
                 last_letter = 'w'               male : femal =     10.6 : 1.0

.. XXX formatting issue above with truncated feature name

This listing shows that the names in the training corpus that end in
"a" are female 38 times more often than they are male, but names
that end in "k" are male 31 times more often than they are female.
...
relevant to the problem.  It's often useful to start with a
...
example, the features returned by the feature extractor shown in
code-gender-features-overfitting_ look like they might be useful.

.. XXX should the gender_features2 docstring be in the caption instead?
...
.. XXX it feels like there's some explanation missing here.
   A simple solution would be to re-order the discussion above.
   First present the "kitchen sink" approach (stopping
   before "However" on line 233).  Then illustrate the approach
   with the pylisting and its performance.
   Then close with the description of overfitting.
   In other words, move the "However...useful" 8 line block
   down to this position and adjust slightly.
...
and a `development`:dt: sub-corpus.  

    >>> test_names = names[:500]
    >>> dev_names = names[500:1500]
    >>> train_names = names[1500:]

.. XXX The terms "training corpus" and "sub-corpus" aren't ideal since
   they're nonstandard and use a term that has extra meaning.
   How about just training data and dev-test data, following
   standard practice (M&S p207, J&M p153)

.. XXX I think development data includes training and dev-test data.
   So its a split: development vs test data at the top level,
   and within development,  training vs dev-test data.

.. XXX A couple of paragraphs later we have the term "heldout data".
   Could use this instead of dev-test, though there's the slight
   complication of its use in heldout estimation.
   "heldout corpus" seems like is a strange concept to me.

The model is trained on the
training sub-corpus, and then run on the development sub-corpus.
...
We can then examine individual cases in the development sub-corpus
where the model predicted the wrong label, and try to determine what
...
accordingly.  The names classifier that we have built generates about
100 errors on the heldout corpus:

    >>> for (tag, guess, name) in sorted(errors): # doctest: +ELLIPSIS +NORMALIZE_WHITESPACE
    ...     print 'wanted=%-8s got=%-8s name=%-30s' % (tag, guess, name)
    wanted=female   got=male     name=Alleen                        
    wanted=female   got=male     name=Allys                         
      ...
    wanted=female   got=male     name=Yoshiko                       
    wanted=male     got=female   name=Abbie                         
    wanted=male     got=female   name=Abby                          
      ...
    wanted=male     got=female   name=Zebadiah                      

.. XXX The wanted= and got= are confusing labels.  From the perspective of
   the human, wanted==gold and got==classifier-output.  From the
   perspective of the system, wanted==classifier-output and got==gold.

.. XXX The "ie" illustration below is contradicted by the Abbie example
   in the above program output.

Looking through this list of errors makes it clear that some suffixes
that are more than one letter can be indicitive of name genders.  For
example, names ending in "el" appear to be predominatly female, while
names ending in "ie" are much more likely to be male.  We therefore
...
the performance on the development data set improves by almost 3
...
different development/training split, to ensure that the classifier
does not start to reflect idisyncracies in the development set.
However, the test set should be kept seperate, and should not be used
for feature development, to ensure that it can be used to accurately
measure the expected performance of the final classifier.

.. XXX It would be nice to have a diagram showing the splitting
   of the data.  An outer box could show the development/test split.
   Within the development box there could be a number of horizontal
   bars, each with a shaded portion indicating a different dev-test
   selection.
...
categories.  For this example, we'll use the movie reviews corpus,
...
indicating whether the document contains that word.  To limit the
number of features that the classifier needs to process, we begin by
constructing a list of the 2000 most frequent words in the overall
corpus:

    >>> all_words = nltk.FreqDist(w.lower() for w in movie_reviews.words())
    >>> word_features = all_words.keys()[:2000]

We can then define a feature extractor that simply checks whether each
of these words is present in a given document:

    >>> stopwords = nltk.corpus.stopwords.words('english')
    >>> def document_features(document):
    ...     document_words = set(document) # [_document-set]
    ...     features = {}
...
.. note:: 
    The reason that we find the set of all words in a document in
    document-set_, rather than just checking if ``word in document``, is
    that checking whether a word occurs in a set is much faster than
    checking whether a word occurs in a list (sec-algorithm-design_).
...
.. XXX The next two paragraphs and two doctest blocks seem unnecessarily
   fragmented.  How about combining them into a single paragraph,
   followed by a single doctest block.  Use doctest callouts to reference
   individual lines if necessary.

Now that we've defined our feature extractor, we can use it to train a
classifier to label new movie reviews:
...
.. XXX How about introducing the term "likelihood ratio"?
...
word for these suffixes:
  ...
  ...         features['endswith(%r)' % suffix] = word.lower().endswith(suffix)
  ...     return features
...
.. XXX Why do we need the single quotes in the feature names?
   This comes from the use of endswith(%r) earlier.  However,
   in example 6-1 we had count(%s) and has(%s).  I don't think
   we want to make feature names look even more like function calls.

Here, we can see that the classifier begins by checking whether a word
ends with a comma |mdash| if so, then it will recieve the special tag
...
then it's most likely a verb (unless it's the word "is"), and if not,
...
---------------------
.. [XXX] Change this to use DecisionTree, to match prev section!!
...
.. XXX How about combining the next three doctest blocks into a pylisting?
   It looks unnecessarily fragmented on the printed page.
   Refer to the various parts of the code using callouts.
...
find the most likely class label for one of the related inputs,
...
In order to use this strategy with general classifier models, we must
...
.. XXX consider incorporating the above doctest block into
   the next pylisting, so that the pylisting is self-contained.
...
.. XXX Some readers will get bogged down in the next two paragraphs.
   How about a new subsection title here: "Other Methods for Sequence Classification"
...
`Maximum Entropy Markov Models`:dt:, and `Linear-Chain Conditional
Random Fields`:dt:,
...
known as `dynamic programming`:dt: (sec-algorithm-design_)
...
.. XXX Section 6.1 is already very long.  How about a new section
   starting here, called "Further Examples of Supervised Classification".
   This will also be convenient to support incoming cross references.
   E.g. add a forward reference to this new section from the
   discussion of Punkt in chapter 3.
...
.. XXX fix the indentation of doctest blocks for consistency with
   the rest of the book (four spaces)
...
.. XXX the term "featuresets" in the above code, then "list of featuresets"
   in the next paragraph, is slightly confusing.  "Using these featuresets, we can..."
...
The ``nps_chat`` corpus, which was demonstrated in
...
.. XXX I don't think "Text" and "Hypothesis" deserve to be capitalized,
   regardless of the practice in the RTE literature.
...
.. XXX the term "compositional and lexical semantics"
   in the next paragraph is odd.  How about "phrasal and lexical semantics".
   Better still, just "semantics"?

.. XXX next paragraph is quite heavy -- how about splitting it?
...
will be no entailment. We let words (i.e., word types) serve as proxies for information, and
...
.. pylisting:: code-rte-features
   :caption: "Recognizing Text Entailment" Feature Extractor.
...
is any consistent pattern within a document |mdash| say, if a given word gets
used with a particular part-of-speech tag especially frequently |mdash| then
...
.. XXX above sentence has "somewhat" and "at least" -- avoid double hedges?
...
.. XXX This section overlaps with development stuff mentioned earlier.
...
.. XXX add xref to kappa in sec-life-cycle-of-a-corpus_
   since the same issue arises when measuring inter-annotator agreement
...
- The `F-Measure`:dt: (or `F-Score`:dt:), which combines the precision
  ...
  `(2*Precision*Recall)/(Precision+Recall)`:math:.
...
.. XXX The [i,j] cells are typeset in italic.
   Use [\ `i`:math:,\ `j`:math:\ ]
...
>>> test = tag_list(apply_tagger(t2, brown.tagged_sents(categories='editorial'))) # doctest: +SKIP
>>> cm = nltk.ConfusionMatrix(gold, test) # doctest: +SKIP
>>> print cm.pp(sort_by_count=True, show_percents=True, truncate=9) # doctest: +SKIP
    |                                                N                 
    |      N      I      A      J                    N      V      C   
    |      N      N      T      J      .      ,      S      B      C   
----+------------------------------------------------------------------
...
.. XXX starting to sound repetitive here; at least acknowledge
   we're repeating ourselves, e.g. "as already mentioned"?
...
.. XXX concept of sub-corpora is odd.  How about using the term "folds".
...
.. XXX next paragraph is quite rough

.. XXX error analysis already covered in enough depth?
   Not sure this section adds anything
...
.. XXX new term in next paragraph "evaluation set" should be "test set"
...
.. XXX Next section isn't long enough to deserve top-level section status. 

.. _sec-classification-methods:
...
.. XXX seems odd to talk about a decision tree "modeling a training corpus" below.
...
.. XXX "best decision stump for the corpus" seems odd.
   How about: "best decision stump for the classification task"?
...
.. XXX the |sum| was not expanded; can't live inside :math: role
...
.. XXX not sure what it means for a decision tree to "model" a
   phylogeny tree since the relationship seems closer than this.

.. XXX "genuine patterns" -> "linguistically significant patterns"?
...
process is illustrated in fig-naive-bayes-bargraph_.
...
.. XXX don't understand "... in ways that don't just reflect differences in the class label"
...
this section.  This simplifying assumption makes it much
...
.. This fig is not referenced in text -- does it need to be?
...
.. XXX introduce the term: "Naive Bayes assumption" or "independence assumption"?
...
.. XXX how about putting these expressions inside an 'ex', for ease
   of reference (if not in the book, by people teaching using the book)
...
.. XXX the |sum| macro isn't processed above
...
.. XXX opening phrase sounds waffly
...
similar to the model used by the Naive Bayes classifier.  But rather
...
.. XXX sum is not typeset properly (this isn't latex)
...
.. XXX need to avoid italic brackets in following paragraph
...
.. XXX column headings are not being typeset in docbook
...
.. XXX "Maximum Classifier"? in following paragraph
...
builds a model that predicts `P(input, label)`:math:, the joint
...
*given* the input value.  Thus, generative models can still be used 
to answer questions 1 and 2.  However, generative models can *not* be
...
.. XXX delete "corpora" from the next sentence?
...
* Supervised classifiers can be used to perform a wide variety of
...
* When training a supervised classifier, you should split your corpus
  into three data sets: a training set, which is used to build the
  classifier model; a development set, which is used to help select
  and tune the model's features; and a test set, which is used to
  evaluate the final model's performance.

* When evaluating a supervised classifier, it is important that you
  use fresh data, that was not included in the training or development
...
* Maximum Entropy classifiers use a basic model that is similar to the
  model used by Naive Bayes; however, it uses iterative optimization
...
Excellent books on using machine learning techniques for |NLP| are [Abney2008]_, [Daelemans2005]_, [Feldman2007]_, [Segaran2007]_, [Weiss2004]_.

Chapter 13 of [Manning2008IR]_ on text classification and Naive Bayes.

Applications of similar methods to:
word sense disambiguation [Agirre2007]_;
parallel texts [Melamed2001]_.

Information on coding numerically intensive algorithms in Python [Kiusalaas2005]_.
