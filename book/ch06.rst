...
Words ending in `-ed`:lx: tend to be past tense verbs (chap-tag_).
Frequent use of `will`:lx: is indicative of news text (chap-words_).
...
In sec-extracting-text-from-corpora_, we saw several examples
...
In chap-tag_ we built a regular expression tagger that chooses
...
by the bigram tagger from sec-n-gram-tagging_, which began by
