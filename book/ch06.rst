...
like a list but does not store all the feature sets in memory:
    ...
    >>> train_set = apply_features(gender_features, names.words()[500:])
    >>> test_set = apply_features(gender_features, names.words()[:500])
...
into the `training set`:dt: and the `dev-test`:dt: set.  
    ...
    >>> train_names = names.words()[1500:]
    >>> devtest_names = names.words()[500:1500]
    >>> test_names = names.words()[:500]
