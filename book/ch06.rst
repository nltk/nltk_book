...
`training set`:dt:, consisting of a list of examples and corresponding
class labels.
...
is about 10% lower than the accuracy of a classifier that only uses
the final letter of each name:
...
we select a `development set`:dt:, containing the corpus data that we
will use to create the model.  This development set is then subdivided
...
used to perform error analysis.  The test set is used for our final
evaluation of the system.  For reasons discussed below, it is
important that we use a seperate dev-test set for error analysis,
...
Having divided the corpus into appropriate data sets, we train a model
...
Using the devtest set, we can generate a list of the errors that the
classifier makes on the dev-test set:
...
the performance on the dev-test data set improves by almost 3
...
does not start to reflect idisyncracies in the dev-test set.

But once we've made use of the dev-test set to help us develop the
...
important to keep the test set seperate, and unused, until our model
development is complete.  At that point, we can use the test set to
check how well our model will perform on new input values.
...
categories.  For this example, we use the movie reviews corpus,
...
will know what information it should make use of
(document-classify-fd_).  For document topic identification, we can
...
.. pylisting:: document-classify-fd
    :caption:
        A feature extractor for document classification, whose
...
classifier to label new movie reviews (document-classify-use_).  To
...
features the classifier found to be especially useful
(document-classify-smif_).

.. pylisting:: document-classify-use
    :caption:
...
This approach is demonstrated in suffix-pos-tag_, which uses a
context-dependent feature extractor to define a part of speech tag
classifier.

.. pylisting:: suffix-pos-tag
...
We let words (i.e., word types) serve as proxies for information, and
...
.. pylisting:: code-rte-features
   :caption: 
       "Recognizing Text Entailment" Feature Extractor.  The
       ``RTEFeatureExtractor`` class, provided by |NLTK|, builds a bag
...
corpus: if we simply re-used the training set as the evaluation
...
dev-test set.  The more similar these two data sets are, the less
...
that difference will be reflected in both the dev-test set and the
evaluation set.  A somewhat better approach is to ensure that
...
A `decision tree`:dt: is a simple flowchart used to choose
...
* When training a supervised classifier, you should split your corpus
  ...
  classifier model; a dev-test set, which is used to help select
...
Consult [URL] for further materials on this chapter.
  For more information on smoothing techniques,
