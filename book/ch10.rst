...
.. note::
   The data illustrated in Table cities_ is drawn from the Chat-80 system
...
and `gefÃ¤llt`:lx: refers to the *please* relation. You
also know that the entity designated by the grammatical subject of
`gefÃ¤llt`:lx: is the stimulus of *please*, while the
...
which determine whether a set of sentences is consistent. This in turn
requires that a logical representation of a sentence `S`:math: helps
make the `truth-conditions`:dt: of `S`:math: explicit. Let's start
...
enclosed by square brackets, then ex-proplog1_ has the
structure 

.. ex:: |phi| ``&`` |psi|

where |phi| and |psi| are sentential
variables and ``&`` is a logical operator corresponding to the English
word `and`:lx:. This structure is the `logical form`:dt: of ex-proplog1_.  

`Propositional logic`:dt: is a system for systematically representing
...
The table boolean-tcs_ specifies the truth-conditions for
propositions containing these operators.
...
.. table:: boolean-tcs
    ...
    +---------------------------------------+--------------+-------------------------------------------------------------+
    ||phi| ``<->`` |psi| is true in *s*     |   iff        ||phi| and |psi| are both false in *s* or both false in *s*   | 
    +---------------------------------------+--------------+-------------------------------------------------------------+

    Truth conditions for boolean operators

These are generally straightforward, though the truth conditions for
...
module. The inputs to the inference mechanism have to be parsed into
logical expressions along the lines ilustrated above.
    ...
    >>> NotQ = lp.parse('-Q')
    >>> R = lp.parse('P -> -Q')
    >>> inference.get_prover(NotQ, [P, R]).prove()
    True
...
operators (i.e, Table boolean-tcs_) and applying them to the values of
...
them, we have to be able look inside basic sentences. This is what we turn to
in the next section.
...
The type-checker has not managed to fully specify the type of
``walk``, since its result type is unknown. To help the
type-checker, we need to specify a `signature`:dt:, which associates
types with non-logical constants. 
...
(pronounced "lambda"), |lambda|\ `x`:math:. The |lambda| counterpart to
...
want to prove (a `proof goal`:dt:) can be derived by a sequence of
...
list of assumptions (which may be empty).
    ...
    >>> prover = inference.Prover9Command(NotQ, [P, R])
    >>> prover.prove()
    True
...
As required, we cannot infer ``north_of(f, s)`` from our premises:
    ...
    >>> inference.Prover9Command(Q, [P, R]).prove()
    False
...
Table nltk_logic_ summarizes the new logical constants of the ``logic``
...
We invoke the Mace4 model builder by calling ``MaceCommand`` in an
analogous way to calling the Prover9 theorem prover. One method is to
...
inconsistent.
    ...
    >>> print inference.MaceCommand(None, [a3, c1]).build_model()
    True
    >>> print inference.MaceCommand(None, [a3, c2]).build_model()
    True
...
Conversely, if ``g`` `is`:em: provable from ``S``, Mace4 may take a long
...
``C1`` is a "skolem constant" that the model builder introduces as a
representative of the existential quantifier |mdash| so it is some
unnamed individual ``'b'``. Now, since our premises said nothing about
the individual constants ``adam`` and ``eve``, the model builder has decided there is no
reason to treat them as denoting different entities, and they both map
...
resulting model is more in accord with our intuitions about the situation.
    ...
    >>> g = lp.parse('love(adam,eve)')
    >>> mb = inference.MaceCommand(g, [a4, a5, a6, a7, a8])
    >>> mb.build_model()
    True
    >>> print mb.convert2val()
    {'C1': 'c',
