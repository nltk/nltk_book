...
various subclasses of ``Expression``:
    ...
    >>> lexpr = nltk.sem.Expression.fromstring
    >>> lexpr('-(P & Q)')
...
.. doctest-ignore::
    >>> lexpr = nltk.sem.Expression.fromstring
    >>> parsed = lexpr('walk(angus)', type_check=True)
    >>> parsed.argument
...
of variables that are free in ``expr``.
    ...
    >>> lexpr = nltk.sem.Expression.fromstring
    >>> lexpr('dog(cyril)').free()
    set()
    >>> lexpr('dog(x)').free()
    {Variable('x')}
    >>> lexpr('own(angus, cyril)').free()
    set()
    >>> lexpr('exists x.dog(x)').free()
    set()
    >>> lexpr('((some x. walk(x)) -> sing(x))').free()
    {Variable('x')}
    >>> lexpr('exists x.own(y, x)').free()
    {Variable('y')}
...
assumptions prove_.
    ...
    >>> NotFnS = lexpr('-north_of(f, s)')  # [_goal]
    >>> SnF = lexpr('north_of(s, f)')    # [_ass1]   
    >>> R = lexpr('all x. all y. (north_of(x, y) -> -north_of(y, x))')  # [_ass2]
...
assignment. Here are a few examples:
    ...
    >>> fmla1 = lexpr('girl(x) | boy(x)')
    >>> m.satisfiers(fmla1, 'x', g)
    {'b', 'o'}
    >>> fmla2 = lexpr('girl(x) -> walk(x)')
    >>> m.satisfiers(fmla2, 'x', g)
    {'c', 'b', 'o'}
    >>> fmla3 = lexpr('walk(x) -> girl(x)')
...
using the ``satisfiers()`` method of ``Model`` objects.
    ...
    >>> fmla4 = lexpr('(person(x) -> exists y.(person(y) & admire(x, y)))')
...
satisfiers for the variable ``y``.
    ...
    >>> fmla5 = lexpr('(person(y) & all x.(person(x) -> admire(x, y)))')
...
person, namely Bruce, who is admired by both Julia and Bruce.
    ...
    >>> fmla6 = lexpr('(person(y) & all x.((x = bruce | x = julia) -> admire(x, y)))')
...
inconsistent.
    ...
    >>> a3 = lexpr('exists x.(man(x) & walks(x))')
    >>> c1 = lexpr('mortal(socrates)')
    >>> c2 = lexpr('-mortal(socrates)')
    >>> mb = nltk.Mace(5)
...
the model that has been built.
    ...
    >>> a4 = lexpr('exists y. (woman(y) & all x. (man(x) -> love(x,y)))')
    >>> a5 = lexpr('man(adam)')
    >>> a6 = lexpr('woman(eve)')
    >>> g = lexpr('love(adam,eve)')
...
(sec-regular-expressions-word-patterns_):
    ...
    >>> lexpr = nltk.sem.Expression.fromstring
    >>> e = lexpr(r'\x.(walk(x) & chew_gum(x))')
    >>> e
    <LambdaExpression \x.(walk(x) & chew_gum(x))>
    >>> e.free()
    set()
    >>> print(lexpr(r'\x.(walk(x) & chew_gum(y))'))
    \x.(walk(x) & chew_gum(y))
...
written in the abbreviated form ``\x y.`` dbl-lambda_.
    ...
    >>> print(lexpr(r'\x.\y.(dog(x) & own(y, x))(cyril)').simplify())
    \y.(dog(cyril) & own(y,cyril))
    >>> print(lexpr(r'\x y.(dog(x) & own(y, x))(cyril, angus)').simplify()) # [_dbl-lambda]
...
string representation parse-drs_.
    ...
    >>> dexpr = nltk.sem.DrtExpression.fromstring
    >>> drs1 = dexpr('([x, y], [angus(x), dog(y), own(x, y)])') # [_parse-drs]
    >>> print(drs1)
    ([x,y],[angus(x), dog(y), own(x,y)])
...
``fol()`` to get a handle on the truth conditions.
    ...
    >>> drs3 = dexpr('([], [(([x], [dog(x)]) -> ([y],[ankle(y), bite(x, y)]))])')
    >>> print(drs3.fol())
...
possible antecedents.
    ...
    >>> drs4 = dexpr('([x, y], [angus(x), dog(y), own(x, y)])')
    >>> drs5 = dexpr('([u, z], [PRO(u), irene(z), bite(u, z)])')
    >>> drs6 = drs4 + drs5
    >>> print(drs6.simplify())
