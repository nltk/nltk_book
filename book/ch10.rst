...
.. XXX note that the next paragraph introduces the term "formula"
...
in ``bark(x)`` is unbound. Consequently it can get bound by some other quantifier,
...
`to the north of`:lx:. However, representing the argument in ex-proplog8_ is
...
.. XXX previous format for this next paragraph broke docbook processing
...
head child's semantics. The two lexical rules just introduce non-logical
...
But ``\y.y(angus)`` only applies to arguments of type  *e*  while ``\x.walk(x)`` is
...
individual, but a functor of type <e, t>. So the type of
...
in the list.
    ...
    >>> for results in nltk.sem.batch_interpret(sents, grammar_file):
...
*semrep* are as before, and *value* is a truth value.
    ...
    >>> grammar_file = 'grammars/book_grammars/simple-sem.fcfg'
    >>> results = nltk.sem.batch_evaluate([sent], grammar_file, m, g)[0]
...
.. XXX "quantifier scope ordering directly reflects scope in the parse
   tree" -- confusing; sounds like the ordering of quantifiers in the sentence
   reflects their relative scope in the parse tree, which is the reverse
   direction to what I expected.  Also, delete "directly"? 

One important limitation of the methods described above is
that they do not deal with scope ambiguity. Instead,
...
.. XXX "constitutes the content of the value of ..."
   -> "is the value of ..." ?
...
documentation can be found at |NLTK-HOWTO-URL|.
...
.. XXX put definition of discourse here (currently an aside,
   five lines into the second paragraph below)
...
quantifier can extend over two or more sentences:
...
whose interpretation depends in part on what preceded them.
...
individual variables of |FOL|.  The conditions apply
...
into a linear format. We represent the general form of a |DRS| as follows:
...
.. XXX next paragraph has the only instance of the term "overload"
   in the book; use a less technical term?
...
.. XXX the inspiration from the CURT system of Blackburn & Bos is more
   appropriate for the further reading section.  Paying dues in
   the text weighs it down, since this won't mean anything to
   people not already familiar with that work.
