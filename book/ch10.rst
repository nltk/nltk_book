...
.. XXX next sentence is hard to parse; idea of representing meaning is novel
...
.. XXX "and to help elucidate our framework for representing meaning"
   -> "and to illustrate our notation" ?
...
detail, let's put the discussion into a broader perspective.
Can a computer understand the meaning of a sentence? And
how could we tell if it did?  This is similar to asking "Can a
computer think?" Alan Turing famously proposed to
answer this by examining the ability of a computer to hold sensible
conversations with a human [Turing1950CMI]_. Suppose you are having a chat session with
...
Turing side-stepped the question of somehow examining the internal
states of a computer by instead using its `behavior`:em: as evidence
...
for natural language understanding in terms of observable behavior.
...
.. XXX "tool in representing certain kinds of reasoning" -> "tool for performing inference" ?
   (and the term "inference" is defined a couple of sentences later, which works fine)
...
.. XXX Montague grammar refs would go in the further reading section
...
.. XXX LogicParser is accessible as nltk.LogicParser, so the
   nltk.sem.logic's LogicParser is overkill
...
.. XXX next sentence talks about the logical rendering of an expression,
   but we've just seen the NLTK rendering.  It seems like a contrast is
   intended, but I assume the logical rendering encompasses the NLTK
   rendering.  How about "NLTK rendering of X" -> "NLTK syntax for X"
...
third-party theorem prover called Prover9 [McCune]_.  A ``Prover9`` instance is
...
.. XXX the above statement about initialization does not match this code.
...
.. XXX inconsistent italicising of parentheses in next para
...
.. XXX why is the term "assignment" in titlecase below?
   Shouldn't it be `assignment`:dt: and ``Assignment``?
...
.. XXX we can use nltk.Assignment()
