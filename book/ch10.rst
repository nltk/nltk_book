...
.. XXX following uses string.join!
...
to a conclusion is called `inference`:dt:. Informally, it is commont to write arguments in a
...
in `not to the north of`:lx: as equivalent to the phrase `it is not the case that
...`:lx:, and translate the latter as the one-place boolean operator ``-`` |mdash| it
applies to one sentence to yield a new one.  So now we can write the implication in
ex-north as follows::

   SnF -> -FnS

How about giving a version of the complete argument?
We'll write ``[A1, ..., An] =>
C`` to represent the argument that conclusion ``C`` follows from assumptions ``[A1,
..., An]``. This leads to ex-proplog5_ as a representation of argument _ex-proplog8_::

    [SnF, SnF -> -FnS] => -FnS

This is a valid argument: if ``SnF`` and ``SnF -> -FnS`` are both true in a situation
*s*, then ``-FnS`` must also be true in *s*. If ``FnS`` were true, this would
...
say a little bit more about this later on.  Logical proofs can carried out with
|NLTK|\ 's ``inference`` module. The inputs to the inference mechanism first have to
be parsed into logical expressions as discussed earlier. (We say more about the
``Prover9()`` theorem prover below.)
...
In order to build logical representations in tandem with syntact parsing, we will use
an additional piece of machinery called the |lambda| calculus. Although this is not
part of |fol|, the two can be combined together nicely. 

Our next step will be to describe how formulas of |fol| are constructed, how the
|lambda| calculus is added, and then how expressions of this representation language
can be evaluated in a model. This will get us to point where we bring everything
together and describe a simple method for constructing semantic representations as
part of the parse process in |NLTK|.
...
The scope of the ``exists x`` quantifier is ``walk(x)``, so the occurrence of ``x``
in ``sing(x)`` is unbound. Consequently it can get bound by some other quantifier,
for example the ``all x`` in the next formula::

    all x.((exists x. dog(x)) -> bark
...
|phi|. Conversely, if ``x`` is free in formula |phi|, then it is `bound`:dt: in ``all
x.``|phi| and ``exists x.``|phi|. If all variable occurrences in a formula are bound, the formula is said
to be `closed`:dt:. 
...
assumptions prove_.
    ...
    >>> prover.prove(NotFnS, [SnF, R]) # [_prove2]  # [_prove]
    True
...
and to combine this with  ``chase(z1,z2)``, the latter needs to first
...
(|DRT|, [KampReyle1993]_) was developed with the specific goal of
...
There are many phenomena in natural language semantics which have not been touched
...
While (i) can be dealt with using |FOL|, (ii) and (iii) require different logics.
...
[Blackburn2005RIN]_ is the first textbook devoted to computational semantics, and
provides an excellent introduction to the area. It expands on many of the topics
covered in this chapter, including underspecification of quantifier scope ambiguity
(e.g., via Cooper storage) and first order inference.
