...
.. TO-DO: 
    ...
    * say something about interpreting first order lambda abstracts
    * fix models

    * DG: dt.retract_sentence('No person dances', quiet=False)".
      Might be best to replace the parameter "quiet" with "verbose"
      since "verbose" is used throughout other modules.
...
.. also accessible as nltk.sem.boolean_ops(), and nltk.boolean_ops()
...
Freedonia`:lx: is the `premise`:dt: of the argument while `Freedonia is not to the
north of Sylvania`:lx: is the `conclusion`:dt:. The step of moving from the premise
to the conclusions is called `inference`:dt:. It is standard to write arguments in a
...
.. XXX problem with negation operator: are we using "-" or |neg| ?
   Changed the paragraph to use |neg|; but two other cases of ``-`` exist.

Here, we have treated the `not`:lx: in `not to the north of`:lx: as
equivalent to the phrase `it is not the case that ...`:lx:.  As a
boolean operator, |neg| applies to one sentence to yield a new one.  If
...
logical expressions along the lines illustrated above. (We say more
...
.. XXX a section overview would be good; its a bit bare going
   straight into syntax.
...
.. XXX definition of "functor" as an expression denoting a function is too obscure
...
We mentioned before that the ``parse()`` method of ``nltk``\ 's
...
The |NLTK| syntax for ex-predlog71_ is ex-forall_.
...
*x* runs away |mdash| but it doesn't say that there are any dogs. So
...
We observed, in sec-proplog_, that propositional logic is too restricted to allow us
to represent constraints like ex-north_ on the meaning of binary predicates such as
`... to the north of ...`:lx:. However, representing the argument in ex-proplog6_ is
straightforward within first order logic. 
Even better, we can perform inference to
show the validity of the argument.  The general case in theorem proving is to
determine whether a formula that we want to prove (a `proof goal`:dt:) can be derived
by a finite sequence of inference steps from a set of assumed formulas. We write this
as `S`:mathit: ⊢ `g`:mathit:, where ``S`` is a (possibly empty) set of assumptions,
and ``g`` is a proof goal. We will illustrate this with |NLTK|'s interface to a
third-party theorem prover called Prover9 [McCune]_.  First, we parse the required
...
assumptions prove_.
    ...
    >>> NotQ = lp.parse('-north_of(f, s)')  # [_goal]
    >>> P = lp.parse('north_of(s, f)')    # [_ass1]   
    >>> R = lp.parse('all x. all y. (north_of(x, y) -> -north_of(y, x))')  # [_ass2]
    >>> prover = nltk.Prover9() # [_prove1]  # [_new-prover]
    >>> prover.prove(NotQ, [P, R]) # [_prove2]  # [_prove]
    True
...
"have the property of walking and chewing gum".  It's often been suggested
...
is often called `β-conversion`:dt:. In order for it to be semantically justified, we
...
complication that we will come to shortly.  In order to carry of |beta|-conversion of
...
by the existential quantifier in the functor expression of ex-alpha2a_? The answer is No.
...
.. XXX just said "formal model".  Should this be "model"?
...
Let's consider a concrete scenario. Our assumptions are the set {`There is a
...
equivalent to ex-sem99_, which is what we wanted all along:
    ...
    >>> vp = ApplicationExpression(tvp, np)
    >>> print vp
...
functors, like quantified ``NP``\ s. Here is the required
...
`y`:mathit:. The structure on right hand is identical, except we have
...
content of the value of ``core``. The rule for ``VP`` just
percolates up the semantics of the  ``IV``, and most of the
...
``store`` and ``core``.
    ...
    ...     print bo
    bo(\P.all x.(girl(x) -> P(x)),z5)
    bo(\P.exists x.(dog(x) & P(x)),z6)

Finally we call ``s_retrieve()`` and check the readings.

    >>> cs_semrep.s_retrieve(trace=True)
    Permutation 1
       (\P.all x.(girl(x) -> P(x)))(\z5.chase(z5,z6))
       (\P.exists x.(dog(x) & P(x)))(\z6.all x.(girl(x) -> chase(x,z6)))
    Permutation 2
       (\P.exists x.(dog(x) & P(x)))(\z6.chase(z5,z6))
       (\P.all x.(girl(x) -> P(x)))(\z5.exists x.(dog(x) & chase(z5,x)))
...
parsed using ``drt.DrtParser`` in place of the default ``LogicParser``.
    ...
    ([x,z10],[Angus(x), dog(z10), own(x,z10)])
...
Inadmissible readings are filtered out by passing the parameter
``filter=True``.
...
I think there should be a note somewhere that while first order logic is very useful
it does not cover all of natural language interpretation, e.g. generalized
quantifiers such as 'most' and intensional constructions.  Perhaps also some
indication of what is not covered here (e.g. tense) that might be accommodated within
fo-logic.

This chapter has not attempted to deal with phenomena such as events, tense and
aspect.

For a wide-ranging, two-volume textbook on logic that also presents contemporary
material on the formal semantics of natural language, including Montague Grammar, see
[Gamut1991IL]_ and [Gamut1991ILG]_. [KampReyle1993]_ provides the definitive account
of Discourse Representation Theory. A more recent and comprehensive study of how the
semantics of many natural language constructions can be represented in a |lambda|\
-based approach, see [Carpenter1997TLS]_.
...
To gain an overview of more advanced contemporary approaches to semantics, try
consulting [Lappin1996HCS]_ or [vanBenthem1997HLL]_.
