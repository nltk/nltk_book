...
:New to Programming?:  The book is suitable for readers with no prior
    knowledge of programming, who are not afraid to tackle new
...
nodes and edges.  For visualizing semantic networks, also install **Graphviz**.
...
.. table:: tab-modules
   ...
   Classification               nltk.classify, nltk.cluster  decision tree, maxent, naive Bayes, EM, k-means  
...
:Simplicity: We have provided an intuitive framework along with
    substantial building blocks, so you can gain a practical
...
:Extensibility: The toolkit easily accommodates new components,
    ...
    extensions should fit into the toolkit's infrastructure.
...
Two possible course plans are illustrated in tab-course-plans.  The first
one presumes an Arts/Humanities audience, while the second one presumes
...
We are indebted to the following people for feedback on
...
Rob Speer,
Richard Sproat.
We are thankful to many students for their comments on
the class materials that evolved into these chapters.
Many others have contributed to |NLTK|, and they are listed at |NLTK-URL|.
...
We thank our partners and families for their patience and support
as we worked on this book during many unsociable hours.
