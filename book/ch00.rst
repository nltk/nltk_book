...
language processing, topics we think all readers should attempt:
tagging, classification, and information extraction (Chapters 5-7).
The next three chapters look at ways to parse a sentence, and recognize
its syntactic structure and semantic content (Chapters 8-10).
The last chapter is devoted to linguistic data and how it can
...
For each chapter, additional materials are available on the |NLTK|
website, at |NLTK-URL|.
...
This book is not a reference work.  Its coverage of |NLP| is selective, and
presented in a tutorial style.  If you have a specific question about Python
syntax, an |NLTK| module, or an |NLP| algorithm, you might find what you're
looking for with the help of the table of contents of the index.  However,
you should also consult the substantial quantity of searchable reference
material available at |PYTHON-URL| and |NLTK-URL|.
...
However, many advanced algorithms are implemented in |NLTK| |mdash|
you can examine the code, and read the documentation and the
research literature that it cites.  The further reading section
and online resources for each chapter include pointers to more
advanced material in the same areas.
