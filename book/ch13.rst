...
After the prompt we've given a name we made up, ``sent``, followed
...
Note that it is standard for computers to sort punctuation
and uppercase letters before lowercase letters. This is thanks
to the underlying numerical representation, which we can see
...
We don't have to type out the lists; we can use short
...
We can count the number of times a word occurs.
...
perform various operations on a text, such as its length, or the
number of times a certain word appears.
...
Slicing is useful when we come to extracting manageable chunks of language from large texts.
Here we have loaded the entire text of *Monty Python and the Holy
Grail*, and have sliced out 25 words:
