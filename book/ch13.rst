...
Python set objects support a variety of standard set operations as listed in
tab-set-methods_.
We can use these to implement a variety of operations on vocabularies,
e.g. unionining multiple vocabularies into a single one,
or finding words that occur in one text but not another, and so on.

.. table:: tab-set-methods
...
same operation on every element of an existing list.
It is similar to a notation from set theory
{`w`:math: | `w`:math: |element| `V`:math: & `P(w)`:math:\ }.
This means "the set of all `w`:math: such that `w`:math: is an
element of `V`:math: (the vocabulary) and `w`:math: has property `P`:math:".
The equivalent Python expression is ``[w for w in V if p(w)]``,
but note that it produces a list, not a set, which means that duplicates are possible.

The preceding examples go through each word in ``sent7``,
assigning each one in turn to the variable ``w`` and
testing whether it meets the relevant condition. So the first
example could be paraphrased: "the list which consists
of all words ``w`` in ``sent7``, as long as the length of ``w`` is less than 4".
In addition to the numerical comparisons shown above, we can also test various properties of words, using the methods listed in tab-word-tests_.
...
Contrast this situation with frequency distributions (sec-overview-of-nltk_),
...
.. note::
   The above example used a `lambda expression`:em:, to be introduced in
   sec-more-on-functions_.  This lambda expression specifies no
...
Let's compare the lexical diversity of the Book of Genesis with the
lexical diversity of the Chat Corpus:
...
You have already encountered several functions, such
as ``len()`` and ``sorted()``. In our running prose, we will
add an empty pair of parentheses after a function name,
to make clear that we are talking about a
...
correct.  (We'll discuss functions at greater length in sec-more-on-functions_.)
...
Functions allow us to abstract away from the details,
to see a bigger picture, and to program more effectively.

Our choice of name for the function makes the
program more readable by others, both where the function is defined,
and where it is used.

Functions help make our programs more reliable.  When we re-use code that has already been developed
...
Python interprets function parameters as values (this is
