...
.. XXX I felt the transition from "you" to "we" and back again in the following text was a bit
.. awkward and so changed the latter to the former.
...
check whether all or any items meet some condition; that is, `all()`` returns
``False`` if its argument list is empty, and otherwise returns ``True``, while
``any()`` returns ``True`` just in the case that its argument is non-empty.
...
.. XXX Following example belongs better at point marked 'XXX **' below
...
.. XXX ** move list(freqdist) here

.. XXX It's unclear what "above code sample" refers to.
.. This passage duplicates what was said in the subsection on sequences and should be
.. (re-)integrated with that earlier text.
...
.. XXX  add "before the value of max() is computed"?  
...
.. XXX really a contrast in levels of abstraction rather than declarative /
.. procedural
...
.. it's used a couple of times in ch03
...
.. XXX slightly amplify the "for reasons that ..."
...
Iteration is an important programming device.  It is tempting to adopt idioms
...
.. note::
   ...
   The ability to generate purmutations of a set of words is
...
.. XXX Whoops we already discussed dictionaries in ch02

(we will learn about dictionaries in sec-dictionaries_).
...
.. XXX do we really want to commit to this URL?
...
.. XXX how about putting this in a pylisting?
...
.. XXX exceptions haven't been explained
...
.. XXX following is first time we've seen "import ... as ..."
...
group average agglomerative clustering, and dendogram plots.
...
.. XXX tuple assignment hasn't been explicitly  explained
...
#. |soso| Write a list comprehension that sorts a list of WordNet synsets for
   ...
   ``minke_whale.n.01``, ``novel.n.01``, and ``tortoise.n.01``, sort them
   according to their ``path_distance()`` from ``orca.n.01``.
...
#. |soso| Read up on "keyword linkage" (chapter 5 of [Scott2006]_).  Extract keywords from
   |NLTK|\ 's the Shakespeare Corpus and using the NetworkX package, plot keyword linkage networks. 
...
.. XXX changed output to display as literal block, but could do with a bit more
.. explanation. 
...
.. XXX is this meant to build on tries?
...
#. |hard|
   Read the article by Hatzivassiloglou and McKeown (1997) on semantic
   orientation of adjectives.  Use the NetworkX package to visualize
