...
In max-comprehension_, storage for the list object must be allocated.  If the text is
...
the function and the expected output.

Docstrings should document the type of each parameter to the function, and the return
type.  |NLTK|\ 's docstrings do this using the "epytext" markup language,
which can be automatically converted into richly formatted,
online API documentation (see |NLTK-URL|).  code-epytext_ illustrates
...
code-trie_ demonstrates the process of building a trie.
...
the Movie Reviews Corpus (code-search-documents_).  By indexing the document collection it can provide much faster lookup.
...
format.  The output is shown in fig-modal-genre_ (a color figure in the online version).
...
* Database Connectivity, e.g. ``mysql-python`` library

Example storing corpus analysis to a database

* PyLucene: Connecting to a text retrieval engine
...
Touched on many topics in programming, and Python programming.
Whole books are devoted to those topics...
...
http://www.python.org/dev/peps/pep-0257/ (docstrings)

Other PEPs?

Algorithms: [Harel2004]_, [Levitin2004]_, [Knuth2006Trees]_

Design:

Hunt, Andrew and David Thomas (2000). *The Pragmatic Programmer: From journeyman to master*  Addison Wesley.
...
#. |soso| Read up on "keyword linkage" (chapter 5 of [Scott2006]_).  Extract keywords from
   |NLTK|\ 's the Shakespeare Corpus and use the NetworkX package, plot keyword linkage networks. 
