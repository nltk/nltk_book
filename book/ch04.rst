...
Recall from the listing in Example stemmer-indexing_ that we used a formatting string
...
Let's use recursion to build a deeply-nested object.
...
by ``virahanka3()`` without cluttering up the program, using a
programming technique that falls outside the scope of this book.

This concludes our brief introduction to dynamic programming.
We will encounter it again in Section sec-parsing_.

.. note::
   Dynamic programming is a kind of `memoization`:dt:.
...
patterns.  For example, in Example modal-tabulate_ we saw a table of numbers
showing the frequency of particular modal verbs in the Brown Corpus, classified
by genre.  The program in Example modal-plot_ presents the same information in graphical
...
.. figure:: ../images/modal_genre.png
   ...
   visualization was produced by the program in Example modal-plot_.
