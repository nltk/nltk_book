...
Some other objects can be converted into a sequence (using ``list()``)
...
an in-place dictionary of keyword arguments ``**kwargs`` (we will learn about dictionaries in sec-dictionaries_).
...
but that it does not complain when it is passed a list.
...
However, this is dangerous because the calling program
...
API documentation (see |NLTK-URL|).  code-epytext_ illustrates
...
all permutations of a list of words.  Notice that we use ``list()``
in order to force the ``permutations()`` function to generate all its output.
