...
.. pylisting:: code-freq-words1
   ...
   'states', 'or', 'united', 'a', 'state', 'by', 'for', 'any', '=', 'president']
...
.. pylisting:: code-freq-words2
   ...
   >>> [word for (word, count) in fd.most_common(20)]
...
to the point that we can do its work with three lines of code:
    ...
    >>> [word for (word, _) in fd.most_common(20)]
    ['the', 'of', 'charters', 'bill', 'constitution', 'rights', ',',
    'declaration', 'impact', 'freedom', '-', 'making', 'independence']

We have used ``_`` as a variable name. This is no different to any other
variable except it signals that we don't have a use for the information
it holds.
...
using ``pylab.savefig()`` pylab-savefig_.  We specify the filename
