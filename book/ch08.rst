...
module.  In Example cfg1_ we define a grammar and show how parse a
...
The grammar in Example cfg1_ contains productions involving various syntactic categories,
...
the grammar shown in Example cfg1_, we end up with two trees, similar to
...
recursion in the grammar in Example cfg1_. However, the grammar in
Example cfg2_ illustrates both kinds of
...
The grammar in Example cfg2_ correctly generates examples like
subcat1_.
...
function in Example wfst_.  We also define a utility function ``display()``
...
The program in Example wfst_ uses this rule to complete the |WFST|.
...
For example, the program in Example sentential_complement_
...
`give`:lx:, as shown in Example give_.
...
where weights appear in brackets, as shown in Example pcfg1_.
...
The grammar in Example pcfg1_ obeys this constraint: for ``S``,
...
#. |soso| Consider the algorithm in Example wfst_.  Can you explain why
...
#. |hard| Write a function that takes a grammar (such as the one defined in
   Example cfg1_) and returns a random sentence generated by the grammar.
