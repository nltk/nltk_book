...
.. XXX can't parse the next sentence "not so far in"?
...
.. XXX new concept of string used in next paragraph, which is likely
   to confuse readers.  Minimally, we should gloss it as "word sequence"
   in a few places.  However, faithfulness to standard terminology is
   not worth any price -- is there an alternative?
   most readers won't go on to read about formal grammar;
   readers who already know about formal grammar will easily
   cope with a different term.

.. XXX content of next paragraph is not strictly necessary so early
   on.  Its already covered by the third paragraph of 8.3.
   Its not long since we gave an overview of this chapter.

In this chapter, we will primarily adopt the formal framework that was
developed within the tradition of "generative grammar" [Chomsky1965]_.
Here, a "language" is considered to be a possibly infinite set
...
We gave an example in chap-corpora_ of how to use the
...
at the string `the worst part and clumsy looking`:lx:. This looks like a `coordinate
...
.. XXX was: In order to make these ideas more formal, we need to examine what is meant by
...
If you are interested in experiment with writing CFGs, you will
...
recursion in the grammar in code-cfg1_. However, the grammar in
code-cfg2_ illustrates both kinds of
...
.. XXX how about moving this section to later in the chapter, so that
   readers see something they can do (i.e. parsing) sooner?
...
contiguous substring of the sentence. fig-depgraph0_ is
...
.. XXX grammar as a string -- different concept of string
   to the rest of this chapter (though I'd prefer to keep
   this concept of string for consistency with the book)
...
.. XXX infinite loop should probably be explained in terms of left-recursive productions
...
list. We have illustrated this in fig-stringpos_.
...
.. XXX above notation conflicts with the earlier explanations of indexing.
   The reason for avoiding the idea of indexes referring to cell boundaries
   is because they do not.  Its best to reinforce the idea that a slice [m:n]
   goes from m to n-1, than teach a falsehood that a slice is a closed interval.
   Let's use a conventional chart diagram if that's what's needed to explain
   the WFST.
...
* Chapters 13-15 of [JurafskyMartin2008]_ on grammars and parsing.
