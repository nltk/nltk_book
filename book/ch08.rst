...
#. How do we represent the grammatical structure of phrases and sentences?
#. How do parsers analyze a sentence and automatically build a
 grammatical representation?

Along the way, we will cover some fundamentals of English syntax, and
...
that "modern English" is not equivalent to the very big
...
In this chapter, the kind of grammars we consider will fall broadly
within the framework of "generative grammar", in which a "language" is
considered to be an enormous collection of all grammatical sentences,
and a grammar is a set of rules can be used for "generating" the
members of this set. Within such frameworks, we usually look for more
than whether a sentence is grammatical or not; we also expect the
grammar to provide us with a representation of the sentence's
structure. We will give examples later of different approaches that
have been adopted for representing grammatical structure.

Although there is an attractive simplicity to saying that a language
is just a set of sentences, it presupposes that the grammaticality is
a Yes/No question. However, observation and experimental research
suggests that humans cannot always give a clear judgement on this
issue; instead, grammaticality seems to be `gradient`:dt:. 

.. Gradience in linguistic data (2005)
   A Sorace, F Keller
   Lingua 115 (11), 1497-1524

To illustrate, the first of the following sentences is clearly
grammatical, and the final ungrammatical, but there is room for doubt
abou the middle two.

.. ex::
  .. ex:: Which friend has Thomas painted a picture of?
  .. ex:: Which friend has Thomas painted the picture of?
  .. ex:: Which friend has Thomas claimed that he painted a picture of her?
  .. ex:: Which friend have Thomas painted a picture of?

The debate around gradience is too extensive for us to explore here,
but it's useful to note that probabalistic grammars seem to capture
the notion well: rather than assigning a shart boundary between
grammatical and ungrammatical, they give us a point somewhere along a continuum.

.. Grammars use recursive `productions`:dt:
...
stack; this is the `shift`:dt: operation.
...
-------------------------------------------------------
Categorial Grammar and Combinatorial Categorial Grammar
-------------------------------------------------------

In this section, we will give a brief introduction to both Categorial
Grammar and a related formalism, Combinatorial Categorial Grammar
(|CCG|). We will start with Categorial Grammar, since it is more
basic.

Categorial Grammar resembles |CFG| in telling us how words combine to
form phrases, and phrases combine to form longer expressions,
including sentences. Also, like |CFG|, a Categorial Grammar defines a
small set of primitive categories such as ``N``, ``NP``, ``PP`` and
``S``. However, the way in which categories are allowed to combine is
determined at the lexical level, not by phrase structure rules. This
is made possible by the introduction of `function categories`:dt:. A
function category resembles a mathematical function in the sense that
its type tells us the category of its argument(s), and the category of
the resulting expression.  Take, for example, the category of a
determiner such as `the`:lex: ``NP/N``. An expression of this type
will combine with an expression of category ``N`` to its right, and
will yield an expression of category ``NP``. The part to the right of
``/`` is the argument, while the part to the left of ``/`` is the
result. Similarly, the category of an intransitive verb is
``S\NP``. This combines with an expression of category ``NP`` to its
left, and gives back an expression of category ``S``. The orientation
of the slash encodes the direction of combination; so ``/``indicates
combination to the left, while ``\`` encodes combination to the right.
...
(These are the `Catalan numbers`:idx:).
