...
In Figure fig-ic-diagram_, we systematically substitute longer sequences
...
In Figure fig-ic-diagram-labeled_, we have added
...
module.  In Example code-cfg1_ we define a grammar and show how parse a
...
The grammar in Example code-cfg1_ contains productions involving various syntactic categories,
as laid out in Table tab-syncat_.
...
the grammar shown in Example code-cfg1_, we end up with two trees, similar to
those we saw for example elephant_:
...
If you are interested in experiment with writing CFGs, you will almost
certainly find it helpful to create and edit your grammar in a text
...
nltk.RecursiveDescentParser(grammar, trace=2)``. You can also check
what productions are currently in the grammar with the command ``for p
in grammar.productions(): print p``.
...
recursion in the grammar in Example code-cfg1_. However, the grammar in
Example code-cfg2_ illustrates both kinds of
...
.. pylisting:: code-cfg2
   ...
   grammar2 = nltk.parse_cfg("""
     ...
     PP -> P NP
     PropN -> 'Buster_Bear' | 'Chatterer' | 'Joe'
     Det -> 'the' | 'a'
...
trees.  Example recnominals_ involves nested nominal phrases,
while recsentences_ contains nested sentences.
...
Although we have only illustrated two levels of recursion, there is no
upper limit on the depth, and you should experiment with parsing
sentences that involve more deeply nested structures. However, be
aware that the recursive descent parser is unable to handle
`left-recursive`:dt: productions of the form ``X -> X Y``. We will
return to this when we discuss parsing in more detail below.
...
relations from heads to dependents.  Figure fig-depgraph0_ illustrates a
...
The arcs in Figure fig-depgraph0_ are labeled with the grammatical
...
contiguous substring of the sentence. Figure fig-depgraph0_ is
...
The grammar in Example code-cfg2_ correctly generates examples like
subcat1_.
...
complements. By contrast with subcat1_, the strings in subcat2_ are ill-formed:
...
In the tradition of dependency grammar, the verbs in Table tab-subcat_ are said
...
ungrammatical examples in subcat2_. In a CFG, we need some way of constraining
...
Table tab-verbcat_ provides more examples of labels for verb subcategories.
...
Six stages of the execution of this parser are shown in Figure fig-rdparser1-6_.
...
Six stages of the execution of this parser are shown in Figure fig-srparser1-6_.
...
collection of possible left corners of that non-terminal. Table tab-lc_
...
the parsing problem.  As we saw in Section sec-algorithm-design_,
...
the main idea in this section; see the Further Reading section at
the end of this chapter for more implementation details.
...
list. We have illustrated this in Figure fig-stringpos_.
...
function in Example code-wfst_.  We also define a utility function ``display()``
...
The program in Example code-wfst_ uses this rule to complete the |WFST|.
...
For example, the program in Example code-sentential-complement_
...
The Prepositional Phrase Attachment Corpus is source of information
about the valency of particular verbs.  We illustrate a powerful
technique for mining this corpus.
...
in Chapter chap-structured-programming_).
...
nouns designating coordinates, as shown in Figure fig-are_.
...
`give`:lx:, as shown in Example code-give_.
...
where weights appear in brackets, as shown in Example code-pcfg1_.
...
The grammar in Example code-pcfg1_ obeys this constraint: for ``S``,
...
* Other PCFG parsers, induction, Normal forms
...
#. |soso| Consider the algorithm in Example code-wfst_.  Can you explain why
...
#. |hard| Write a function that takes a grammar (such as the one defined in
   Example code-cfg1_) and returns a random sentence generated by the grammar.
