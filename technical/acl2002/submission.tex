...
% We assume that the user is familiar with the acronym ``NLP;'' is
% that ok?
...
% [EL] ``functionality'' isn't a great word, here.
...
% Maybe ``challenging requirements''?
...
% [EL] This was too ambiguous for me (e.g., we discuss Python)
...
from \url{http://nltk.sf.net}.  NLTK runs on most platforms, including
...
% [EL] is ``desiderata'' an acceptable word here?
...
% [EL] ``punitive'' like a strange word.
...
documented.  
%Three documentation types are necessary: tutorials, to
...
% available?  (Currently, 8 tutorials, mean=16pp, range=6pp-52pp.)
...
from the corse website \url{http://www.cis.upenn.edu/~cis530/}.
...
% Where does this go??  Leaving it out for now..
...
was finding large clean corpera that the students could use for their
assignments.  Several of the students needed assistance finding
suitable corpera for their final projects.  Another issue was the fact
...
Finding suitable corpera is a prerequisite for many studnet
assignments and projects.  We are therefore putting together a
standard collection of large, clean, open-source corpera, containing
corpera appropriate for every module defined by the toolkit.
