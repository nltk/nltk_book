...
class DocBookTranslator(nodes.NodeVisitor):
    ...
    def visit_figure(self, node):
        ...
        self.body.append(self.starttag(node, 'informalfigure', **atts))
        #self.body.append('<blockquote>')
    ...
    def visit_section(self, node):
        if self.body and 'chapter' in self.body[-1]:
            raise nodes.SkipNode
        else:
    ...
    def visit_table(self, node):
        ...
        if 'ids' in node.attributes and node.attributes['ids']:
            atts['id'] = node.attributes['ids'][0]
        self.body.append(self.starttag(node, 'table', **atts))
